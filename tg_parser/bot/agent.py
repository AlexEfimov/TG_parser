...
# read-only report shape instead. Stripped when the deterministic resume path
# re-issues the call to obtain the real preview. A new report-only flag on any
# confirm-gated write tool MUST be registered here.
_PREVIEW_SUPPRESSING_ARGS: frozenset[str] = frozenset({"dry_run"})


def _write_intent_snapshot(
    unpreviewed_write_calls: list[tuple[str, dict[str, Any]]],
) -> dict[str, Any]:
    """Snapshot the last preview-LESS write call for the handler (#359).

    The framework owns confirmation: a write tool's ``{"preview": True, ...}``
    payload is the ONLY thing that arms ``ConfirmFlow``. A confirm-gated write
    tool can still end a turn with nothing armed — a ``dry_run`` report, a
    BUG-009 rejection of an LLM-issued ``confirm=true``, an executor error — and
    the user's «да» then hits ``current_state is None`` and dead-ends on the
    opaque «Я не совсем понимаю ваш ответ» (BUG-046 / BUG-031 / BUG-009).

    The first repair (BUG-086) PREDICTED that ask from the LLM's own prose. Prose
    is an open class: six false-positive vectors in four review rounds, three of
    them purely morphological, and two — free Russian word order and text the
    LLM merely QUOTED from the knowledge base — out of reach of any regex. So
    the framework no longer reads the turn's text at all (ADR-0020): it hands the
    handler this snapshot, and the trigger becomes the user's own next message
    classified against a CLOSED token vocabulary.

    Args are sanitized HERE rather than at re-issue time, so the mutating shape
    cannot be reconstructed from FSM storage even by accident:

    * ``confirm`` is dropped unconditionally (BUG-009 — only the FSM
      confirm-turn may set it);
    * every ``_PREVIEW_SUPPRESSING_ARGS`` flag is dropped, otherwise the
      re-issue would return the same report-only payload and never a preview.
    """
...
def _write_intent_or_none(
    preview_pending: dict[str, Any] | None,
    unpreviewed_write_calls: list[tuple[str, dict[str, Any]]],
) -> dict[str, Any] | None:
    """Snapshot the pending write intent for a terminal ``AgentResult`` (#359).

    Shared by EVERY exit that carries ``preview_pending``, so the two cannot
    drift apart: a run that exhausts ``MAX_AGENT_TURNS`` after a preview-less
    write call left the user in exactly the dead-end this mechanism exists to
    close, and the exhaustion exit already propagates ``preview_pending``.

    An armed preview wins — ``ConfirmFlow`` is the stronger, already-shown
    contract, and the handler drops the snapshot when an FSM is armed anyway.
    """
    if preview_pending is not None or not unpreviewed_write_calls:
        return None
    return _write_intent_snapshot(unpreviewed_write_calls)
...
class AgentResult:
    ...
    # BUG-086 / #359 (ADR-0020): a confirm-gated write tool ran this turn and
    # armed NO preview, so the user's «да» would dead-end. The handler persists
    # this snapshot and — if the very NEXT message is a bare affirmative token —
    # re-issues the tool in its preview shape and arms ``ConfirmFlow`` then.
    # The turn's own TEXT is never consulted: the trigger is the user's closed
    # vocabulary reply, not an inference from LLM prose.
    #
    # ``write_intent_pending`` shape::
    #
    #     {"tool_name": str, "args": dict}  # args sans confirm / dry_run
    write_intent_pending: dict[str, Any] | None = None
    ...
    async def process_message(
        ...
        for turn in range(MAX_AGENT_TURNS):
            ...
            if not function_calls:
                ...
                # BUG-086 / #359: a confirm-gated write tool ran and armed no
                # preview. Hand the handler a snapshot of that call — the state
                # is read off the CALL, never off ``response_text``.
                return AgentResult(
                    ...
                    write_intent_pending=_write_intent_or_none(
                        preview_pending, unpreviewed_write_calls
                    ),
                )
