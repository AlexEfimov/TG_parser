...
class RawTelegramMessage(BaseModel):
    ...
    id: str = Field(
        description="Уникальный идентификатор сообщения (message_id как строка)"
    )
    message_type: MessageType = Field(
        description="Тип сообщения: пост канала или комментарий"
    )
    ...
    date: datetime = Field(
        description="Время отправки сообщения (UTC)"
    )
    text: str = Field(
    ...
    parent_message_id: Optional[str] = Field(
        None,
        description="Для комментария: id родительского сообщения"
    )
    thread_id: Optional[str] = Field(
        None,
        description="Идентификатор треда/обсуждения"
    )
    language: Optional[str] = Field(
        None,
        description="Определённый язык сообщения"
    )
    raw_payload: Optional[Dict[str, Any]] = Field(
        None,
        description="Объект из Telegram-клиента для отладки (может быть усечён до 256KB)"
    )
...
class Entity(BaseModel):
    ...
    confidence: Optional[float] = Field(None, ge=0.0, le=1.0, description="Уверенность")
...
class ProcessedDocument(BaseModel):
    ...
    id: str = Field(
        description="Уникальный id (детерминированный от source_ref)"
    )
    source_ref: str = Field(
        pattern=r"^tg:[^:]+:(post|comment):[^:]+$",
    ...
    source_message_id: str = Field(
        description="Ссылка на исходный RawTelegramMessage.id"
    )
    channel_id: str = Field(
        description="Идентификатор канала/чата"
    )
    processed_at: datetime = Field(
        description="Время последнего успешного формирования/обновления"
    )
    ...
    summary: Optional[str] = Field(
        None,
        description="Краткое резюме, сгенерированное ИИ"
    )
    topics: List[str] = Field(
        default_factory=list,
        description="Основные темы/категории (может быть пустым)"
    )
    ...
    language: Optional[str] = Field(
        None,
        description="Определённый язык (вычисляется на этапе processing)"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None,
        description="Метаданные обработки (pipeline_version, model_id, prompt_id, parameters)"
    )
...
class Anchor(BaseModel):
    ...
    anchor_ref: str = Field(
        pattern=r"^tg:[^:]+:(post|comment):[^:]+$",
        description="Каноническая ссылка на материал"
    )
    score: Optional[float] = Field(
        None,
        ge=0.0,
        le=1.0,
        description="Оценка релевантности/якорности"
    )
...
class TopicCard(BaseModel):
    ...
    id: str = Field(
        description="Устойчивый id темы (topic:<primary_anchor_ref>)"
    )
    title: str = Field(
    ...
    scope_in: List[str] = Field(
        min_length=1,
        description="Что относится к теме"
    )
    scope_out: List[str] = Field(
        min_length=1,
        description="Что не относится к теме"
    )
    ...
    anchors: List[Anchor] = Field(
        min_length=1,
        description="Якорные первоисточники (уникальны по anchor_ref)"
    )
    sources: List[str] = Field(
        min_length=1,
        description="Список источников (каналов)"
    )
    updated_at: datetime = Field(
        description="Момент последнего обновления карточки"
    )
    ...
    related_topics: Optional[List[str]] = Field(
        None,
        description="Ссылки на близкие темы (по id)"
    )
    status: Optional[str] = Field(
        None,
        description="Статус темы для workflow"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None,
...
class BundleItem(BaseModel):
    ...
    role: BundleItemRole = Field(
        description="Роль материала: якорный или поддерживающий"
    )
    ...
    justification: Optional[str] = Field(
        None,
        description="Объяснение, почему материал включён"
    )
...
class TopicBundle(BaseModel):
    ...
    items: List[BundleItem] = Field(
        min_length=1,
        description="Материалы, входящие в подборку"
    )
    updated_at: datetime = Field(
    ...
    time_range: Optional[TimeRange] = Field(
        None,
        description="Опциональный диапазон дат (для snapshot)"
    )
    channels: Optional[List[str]] = Field(
        None,
        description="Список каналов, материалы которых вошли в подборку"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None,
...
class KnowledgeBaseEntrySource(BaseModel):
    ...
    type: str = Field(
        description="Тип источника: 'telegram_message' или 'topic'"
    )
...
class KnowledgeBaseEntry(BaseModel):
    ...
    source: KnowledgeBaseEntrySource = Field(
    ...
    title: str = Field(
        description="Краткий заголовок/название"
    )
    content: str = Field(
    ...
    topics: List[str] = Field(
        default_factory=list,
        description="Темы/категории для фильтрации"
    )
    tags: List[str] = Field(
        default_factory=list,
    ...
    vector: Optional[List[float]] = Field(
        None,
        description="Опциональное векторное представление"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None,
