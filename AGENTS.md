...
- MCP-сервер, привязанный к машине: локальный dev-инстанс `tg-parser` (прод-эндпоинт доступен из обоих режимов).
...
- **Sourcegraph отключён 2026-08-13 как избыточный.** Его сценарий — навигация по структуре кода — закрывает graphify, локально и детерминированно, а документацию библиотек берёт Context7. Возвращать имеет смысл только под поиск по **чужим** кодовым базам; для одного репозитория это лишняя зависимость с интерактивным OAuth, который к тому же непроходим в облаке.
