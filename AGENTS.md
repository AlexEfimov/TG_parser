...
- Правила в `.cursor/rules/` и сабагенты в `.cursor/agents/` — единственная агентская конфигурация, которая доезжает до **обоих** режимов; всё в `~/.cursor` остаётся на машине. Поэтому `.gitignore` держит оба каталога отслеживаемыми, а `mcp.json` — намеренно нет (в нём ключи).
