...
class TestCreateWatchlist:
    ...
    async def test_target_channel_happy_path(self, app, client, user_repo):
        ...
        response = await client.post(
            ...
            json={
                "title": "MiCA channel",
                "channel_ids": ["crypto_news"],
                "target": {"kind": "channel", "channel_id": "@regwatch"},
                "keywords": ["mica"],
                "threshold": 0.6,
            },
        )
        ...
        body = response.json()
        assert body["created"] is True
        assert body["target"] == {"kind": "channel", "channel_id": "@regwatch"}

    async def test_target_chat_explicit_happy_path(self, app, client, user_repo):
        """``target={kind:chat, chat_id:N}`` is the canonical form of legacy
        ``chat_id=N`` and must yield the same row shape."""
        owner = await user_repo.create_user("alice_target_chat_wl")
        _override_user(app, _user(owner.id))
        ...
        response = await client.post(
            ...
            json={
                "title": "MiCA",
                "channel_ids": ["crypto_news"],
                "target": {"kind": "chat", "chat_id": 99999},
                "keywords": ["mica"],
                "threshold": 0.6,
            },
        )
        ...
        assert response.status_code == 201, response.text
        assert response.json()["target"] == {"kind": "chat", "chat_id": 99999}

    async def test_chat_id_and_target_conflict_returns_422(self, app, client, user_repo):
        """Mutual exclusion: both legacy + new fields is a validation error."""
        owner = await user_repo.create_user("alice_conflict_wl")
        _override_user(app, _user(owner.id))
    ...
    async def test_neither_chat_id_nor_target_returns_422(self, app, client, user_repo):
        ...
        owner = await user_repo.create_user("alice_neither_wl")
        _override_user(app, _user(owner.id))
    ...
    async def test_idempotent_replay_with_target_channel(self, app, client, user_repo):
        """Same (owner_id, title) with ``target=channel`` replays as
        ``created=False`` and preserves the channel target."""
        owner = await user_repo.create_user("alice_idem_target_wl")
        _override_user(app, _user(owner.id))
        ...
        assert second.json()["target"] == {"kind": "channel", "channel_id": "@regwatch"}
