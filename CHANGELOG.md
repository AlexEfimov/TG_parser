...
- **Bot logging (BUG-088)** — `fsm_confirm_unknown_token` больше не пишет ответ
  ...
  набор закреплён литералом в тесте. Слайс дополняет BUG-087, а не расширяет
...
- **Tests** — `tests/test_bot_confirm_unknown_log_088.py`: privacy-пин идёт
  ...
  таблица вердиктов и пин, который мутирует каждую живую запись whitelist на
  одну правку. Проверено мутацией: с возвращённым `logger.info` семь тестов из
  двадцати красные, сообщение печатает утёкшую запись.
