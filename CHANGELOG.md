...
**Пересчёт `degraded` на проде не выполнен:** в локальном режиме прод недоступен по замыслу (`dev_doctor` не находит ни `ssh prod`, ни prod-MCP), а деплой ждёт GO владельца. Проверять после деплоя: источник, у которого в тике были только дубликаты, не в `degraded`, его `fail_count` = 0 и `last_error` = NULL.
