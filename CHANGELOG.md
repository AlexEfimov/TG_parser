...
### Deploy (docs-only) — прод на `f005f93`, включая аудит документации PR #399 (2026-08-12 17:40 UTC)

**Что на проде.** `f821e53 → f005f93` шестью последовательными fast-forward'ами
(reflog прода: 13:09, 13:39, 13:53, 14:47, 15:01 и 17:40 UTC). Последний из них,
17:40 UTC, привёз PR [#399](https://github.com/AlexEfimov/TG_parser/pull/399)
(аудит документации, сессия #2) вместе с PR #400. Всего в окне
`f821e53..f005f93` — PR #391–#400 (`git log --oneline f821e53..f005f93`).

**Сборка и пересоздание не требовались, и это проверено, а не предположено.**
Ни один из восьми файлов PR #399 не читается приложением: все восемь — markdown
(`docs/**` и корневые `*.md`), а единственное, что приложение грузит с диска, —
`prompts/*.yaml`. Упоминания `docs/notes/BUG_LOG.md` в коде — текст docstring'ов,
не чтение файлов. Соответственно `CREATED` контейнеров `tg_parser` /
`tg_parser_mcp` / `tg_parser_bot` остался `2026-08-12 13:11 UTC`, то есть они
пережили пять последующих `git pull` нетронутыми; api `/health` 200.

**Содержимое сверено по файлам, а не по SHA** — SHA доказывает, что коммит
дошёл, но не что дошёл нужный текст: на проде `AUDIT_DOCUMENTATION_2026-08-12.md`
328 строк, битых `../../`-ссылок в `USER_GUIDE` ноль, старого счётчика
«Tools: 43» в `MCP_AGENT_GUIDE` нет, баннер в `current-state.md` на месте,
`DEVELOPMENT_ROADMAP` ведёт на `ROADMAP_KARPATHY_LIKE_LIVING_KB.md`.

> **Почему эта запись появилась задним числом.** Прод подтягивали **восемь** раз
> за день; записаны были два первых (`1a51441 → 17306d0` и `17306d0 → f821e53`),
> остальные шесть прошли молча. На момент проверки CHANGELOG отставал от прода
> на десять PR. Ровно тот класс расхождения, который каталогизировала сессия #2:
> документ не врёт в написанном, он молчит о случившемся. Дешёвая мера — брать
> состояние прода командой (`ssh prod 'cd /home/user/TG_parser && git log -1'`),
> а не из последней записи в CHANGELOG.

### Ops — прод подтянут до `main` и убраны копии `.env` из корня репозитория (2026-08-12 11:55 UTC)

**Деплой (docs-only).** Прод `17306d0 → f821e53` чистым fast-forward: PR #388–#390
трогают только `CHANGELOG.md`, `docs/notes/BUG_LOG.md` и prep-документ планирующей
сессии. Ни сборки образа, ни пересоздания контейнеров, ни `db upgrade` не
потребовалось — и это проверено, а не предположено: `CREATED`/uptime всех
контейнеров после `git pull` те же, api `/health` 200.

**Уборка.** В корне `/home/user/TG_parser` лежали 13 копий `.env` и один
`docker-compose.yml.bak-bug028-20260523-114830` — осадок fix-сессий с июня по
август. Все untracked, на работу не влияли, но **девять из них имели права `664`**,
то есть секреты были читаемы любым пользователем хоста.

Файлы не удалены, а перенесены в архив
`/home/user/backups/tg_parser_repo_root_backups_20260812.tar.gz` (`600`), после
чего оригиналы сняты. Порядок был именно такой: архив создан, распакован во
временный каталог и сверен `sha256sum -c` по всем 14 файлам — и только после
успешной сверки originals удалены. Восстановление — распаковкой архива.

Живой `.env` в список не входил (sha до и после совпадает), `.env.example` и
`docker-compose.yml` на месте; `git status` в корне репозитория теперь пуст.

> Копии секретов после этой уборки продолжают существовать — одним файлом с
> доступом только владельцу. Это уборка sprawl'а, **не** ротация ключей: если
> какой-то из ключей считается скомпрометированным из-за прежних прав `664`,
> ротация — отдельное решение.

### Deploy — прод приведён в соответствие с `main` (2026-08-12 07:19 UTC)

Раскатан pre-Wave-3 заход (PR #384–#387). Прод: `1a51441 → 17306d0`, образ пересобран
`sha256:9416d99c → sha256:59326081` (09:18 CEST), три контейнера, делящие его —
`tg_parser`, `tg_parser_mcp`, `tg_parser_bot` — **пере-созданы** (re-create, не
`restart`: BUG-078) в 07:19:55 UTC, все `healthy`. Точка отката: тег
`tg_parser:pre-20260812` на прежний образ — снят **до** сборки, потому что
`latest` при сборке перезаписывается и иначе откатываться было бы некуда.

#### Что деплой доставил

- **Наблюдаемость обхода advisory-lock** — три места, где отказ
  `Database.get_instance()` тихо отдавал `yield True`, то есть межпроцессный лок
  не брался, а вызывающий код продолжал считать, что держит его. Поведение не
  менялось; появился `warning` с классом ошибки на аномальной ветке и `debug` на
  обычной no-DB. Проверено в работающем контейнере: все три сайта живые.
- **Удаление трёх мёртвых bind-mount'ов `./.env:/app/.env:ro`** (BUG-092). В
  контейнере `/app/.env` больше не существует — и это главная проверка деплоя:
  конфигурация продолжает доезжать (`retention_days=180`, `keep_last_n=50`,
  `max_age=21` читаются из OS-env), то есть маунт действительно был декоративным,
  как и утверждал разбор.
- Всё остальное из #384–#387 — документация, тесты и CI-job; прода не касается.

#### Проверено после пересоздания

- Код читался **через `/opt/venv/bin/python3`** — тот интерпретатор, которым
  работает entrypoint. Плоский `docker exec … python -c` для этого не годится
  (BUG-092: с `cwd=/app` он импортирует bind-mounted дерево и показывает не то,
  что исполняется).
- `processing_concurrency = 5` — не сдвинулось, как и решено: ключ намеренно вне
  compose allow-list.
- Живые эндпоинты: api `/health` 200, mcp `/health` 200, публичный api 200,
  публичный mcp 401 (auth-gate). Ноль `traceback` / `critical` в логах обоих
  сервисов после старта.
- Scheduler перерегистрировал cron'ы, включая `topic_card_versions_purge` —
  retention пережил пересоздание.
- Бот: 35 инструментов, redaction (BUG-087) и truncation (BUG-088) на месте,
  digest-планировщик поднялся с 4 активными подписками.
- **Пробник периметра — 16/16 зелёных** после пересоздания (апстрим-порты не
  сдвинулись, `/metrics` → 403, стриминг MCP цел, сертификаты 79 дней).
  `~/backup-alert.log` остался 0 байт.

Ожидаемый побочный эффект: гейдж `tg_topic_card_versions_rows` обнулён до
следующего тика 03:30 UTC — его пишет только on-path ветка purge. Доказательство
первого тика уже зафиксировано в [ADR-0018](docs/adr/0018-topic-card-versions-retention.md)
и в истории Prometheus.

### Docs — ADR-0021 переведён в `Accepted`: Т-6 закрыт второй копией (2026-08-10)

#### Changed

- **[ADR-0021](docs/adr/0021-backup-and-recovery-requirements.md) → `Accepted`.**
  Владелец выбрал для Т-6 вариант «вторая копия owner-side»: DR-репозиторий в
  отдельном бакете B2, ключ доступа к которому живёт **только** на машине
  владельца и на сервере не появляется. Ключ хоста привязан к первому бакету и
  второго не видит, поэтому взломщик, получивший сервер, основной репозиторий
  уничтожить может, а вторую копию — нет. Это ветка «раздельные креды» из
  формулировки требования.
- Проверено: DR-репозиторий содержит те же 6 снапшотов при **побайтово
  совпадающем** размере — 310.265 MiB, 882 блоба, 1.319 GiB несжатого, сжатие
  4.35×. Совпадение доказывает, что `--copy-chunker-params` сработал: при
  несовпадении границ чанков дедупликация между репозиториями не работала бы и
  объём заметно вырос бы.
- Зафиксировано честное ограничение: вторая копия обновляется вручную раз в
  месяц, то есть её RPO — **до месяца** против 24 часов у основного
  репозитория. Автоматизировать её с сервера невозможно, не положив туда креды
  второго бакета, что уничтожило бы саму защиту.
- [`docs/technical-debt-roadmap.md`](docs/technical-debt-roadmap.md) § 7 закрыт.

### Docs — ADR-0021: реализация проверена, Т-6 остаётся открытым (2026-08-10)

#### Changed

- **[ADR-0021](docs/adr/0021-backup-and-recovery-requirements.md)** — добавлен
  `Update 2026-08-10`. Инфраструктурный слой выполнил задание; утверждения его
  отчёта **проверены независимо** со стороны продукта, расхождений нет.
  Т-1…Т-5 закрыты: restore drill пройден по записанной процедуре (возраст копии
  **15 ч** при RPO ≤ 24 ч, восстановление **101 с** при RTO ≤ 8 ч, счётчики
  сошлись с манифестом снапшота, а не с живым продом), `restic check` без ошибок,
  сигнал свежести определяет валидность через `restic check`, а не по факту
  запуска скрипта.
- **Статус остаётся `Proposed`.** Т-6 (устойчивость к компрометации хоста)
  **не выполнен**: через S3 ключ хоста обязан иметь `deleteFiles`, а удаление с
  `versionId` необратимо — проверено экспериментом на канареечном объекте.
  Версионирование бакета защищает от случайного удаления, но не от
  целенаправленного взломщика. Промежуточная версия документации утверждала
  обратное; утверждение снято как ложное. Перевод в `Accepted` — после выбора
  владельцем одного из трёх вариантов, описанных в ADR.

### Docs — ADR-0021: граница ответственности по бэкапам (2026-08-07)

#### Added

- **[ADR-0021](docs/adr/0021-backup-and-recovery-requirements.md)** (`Proposed`) —
  резервное копирование и offsite-хранение объявлены **вне скоупа репозитория
  продукта**: это задача инфраструктурного слоя всего сервера, где живут и
  посторонние стеки, и общий системный nginx. Втягивание их сюда означало бы
  чужие конфиги в репозитории и вторую копию конфигурации, расходящуюся с `/etc`
  — тот же класс, что BUG-090.
  Вместо этого ADR задаёт **контракт**: классы данных с их восстановимостью и
  измеренной стоимостью повторного создания ($215–380 за переобработку и
  топикизацию, если не бэкапить производное), объявленные продуктом RPO ≤ 24 ч и
  RTO ≤ 8 ч, шесть требований к инфраструктуре (восстановление на чистой машине,
  клиентское шифрование, целостность дампа, машиночитаемый сигнал свежести,
  заметность отказа, устойчивость к компрометации хоста) и четыре встречные
  гарантии продукта (стабильный формат дампа, единственное место секретов,
  спецификация периметра, схема данных).
  Перевод в `Accepted` — после пройденного restore drill.
...
- **[`ops/backup-nginx-config.sh`](ops/backup-nginx-config.sh)** — еженедельный
  ...
  **Сигнал об изменении конфигурации:** при расхождении с предыдущим архивом
  скрипт пишет в лог `CHANGED` со списком затронутых файлов и кладёт полный diff
  в `changes/`. Это дешёвая замена ревью — отвечает на «кто-то поменял, надо
  посмотреть», не заводя вторую копию конфига, которая разошлась бы с
  `/etc/nginx` (тот же класс, что BUG-090). Волатильные файлы (сроки
  сертификатов, состояние сервисов) из сравнения исключены, иначе тревога
  срабатывала бы каждую неделю сама по себе; первый запуск изменением не
  считается. Источники `NGINX_SRC`/`LE_SRC` сделаны переопределяемыми, чтобы эту
  ветку можно было проверить на копии, не имея прав на запись в боевой `/etc`.
...
- **Bot logging (BUG-088)** — `fsm_confirm_unknown_token` больше не пишет ответ
  пользователя в лог. Поле `normalized` несло **весь** текст реплики целиком
  (`" ".join(text.split()).casefold()`), а попадает в эту ветку любой свободный
  текст на вооружённом ConfirmFlow — включая вставленный credential. Вместо
  значения запись теперь несёт факты о нём: закрытый `verdict` плюс
  `length` / `token_count` / `is_single_token` / `has_digits` / `has_punct`, и
  новое поле `tool` — какое именно pending-действие осталось без ответа (его у
  события не было вовсе, в отличие от трёх соседей). Ключей ровно восемь,
  набор закреплён **собственным** литералом в тесте — не константой модуля,
  иначе обе стороны сверялись бы с одним источником (см. bullet про тесты
  ниже). Слайс дополняет BUG-087, а не расширяет
  его: тот deny-list по именам аргументов до свободного текста не достаёт по
  построению — имени аргумента, которое можно запретить, здесь просто нет.
- **Bot logging (BUG-088)** — поле нельзя было просто удалить: оно единственный
  диагностический payload события и существует ради BUG-032 (оператор должен
  видеть, ПОЧЕМУ токен отвергнут). `verdict` сохраняет ответ на этот вопрос из
  закрытого словаря пяти значений: `near_miss_affirmative` /
  `near_miss_negative` («дя», «(да)» — опечатка или украшение, прямой кандидат
  на расширение whitelist), `single_token_unlisted` (одно слово мимо обоих
  списков; при большом `length` и `has_digits` это форма вставки),
  `multi_token_free_text`, `non_text` (пусто / emoji / пунктуация). Цена
  осознанная и записана в BUG_LOG: КАКОЙ именно синоним написал пользователь,
  из лога больше не узнать. Отвергнуты truncation (утекает короткие секреты и
  коррелируемый префикс), hash (несолёный обратим словарём) и dump сырого
  текста на DEBUG (этот прецедент уже откатывался — `b0dcef3` / `8332aa3`).
- **Bot logging (BUG-088)** — helper вынесен в новый
  `tg_parser/bot/confirm_unknown_log.py` (stdlib, без новых зависимостей;
  Levenshtein ≤ 1 — локальная функция). Whitelists `AFFIRMATIVE_TOKENS` /
  `NEGATIVE_TOKENS` не копируются: helper читает их из `handlers` локальным
  импортом внутри функции (модульный был бы циклом), поэтому near-miss всегда
  считается против тех же frozenset'ов, что использует
  `classify_confirmation_token`. Поведение хендлера не менялось: текст ответа
  пользователю бит-в-бит тот же, FSM остаётся вооружённым, порядок ветвей
  прежний.
- **Tests** — `tests/test_bot_confirm_unknown_log_088.py`: privacy-пин идёт
  через **настоящий** `_handle_confirmation_response`, а не через копию
  конфигурации, и требует, чтобы событие **существовало** — пустая запись
  читалась бы как «privacy pass», уничтожив диагностику BUG-032. Секрет
  сверяется **casefold'ом**: сайт до фикса folds реплику, и обычная проверка
  подстроки пропустила бы собственный дефект (проверено — именно этот assert и
  ловит утечку). Плюс source-tripwire на исчезнувшее выражение и на DEBUG-dump,
  таблица вердиктов и пин, который мутирует каждую односимвольную alnum-запись
  живых whitelist'ов на одну правку. Проверено мутацией: с возвращённым
  `logger.info` семь тестов из двадцати красные, сообщение печатает утёкшую
  запись.
- **Tests (review hardening)** — key-set-гард читал **обе** стороны из
  тестируемого модуля: запись сверялась с `UNKNOWN_CONFIRM_LOG_KEYS`, то есть с
  константой, которую правит тот же коммит, что добавляет поле. Дефект показан
  мутацией: `"preview": normalized[:4]` в helper'е **плюс** `"preview"` в
  константе — все двадцать тестов зелёные, а четырёхсимвольный префикс каждой
  отвергнутой реплики уходит на INFO. Это ровно тот дефект гарда, который
  BUG-089 нашёл у себя одним PR ранее. Набор ключей теперь пинится независимым
  литералом `_EXPECTED_LOG_KEYS` в тесте, а `UNKNOWN_CONFIRM_LOG_KEYS`
  сверяется с ним отдельным assert'ом; та же мутация теперь красная. Словарь
  вердиктов пинится равенством с набором, который **фактически производит**
  таблица входов, — новая строка во frozenset'е без достижимого входа больше не
  пройдёт незамеченной. Source-tripwire'ы (`normalized=` и DEBUG-dump) сужены с
  всего ~2900-строчного `handlers.py` до тела `_handle_confirmation_response`
  через `inspect.getsource` — module-wide форма ложно падала бы на любом
  несвязанном `logger.debug` / `f(normalized=…)`, а `logger.debug` — обычная
  идиома соседних bot-модулей; заодно ловится и `log.debug(…)` / bound logger,
  которые узкая форма пропускала. Путь к `handlers.py` — repo-relative
  (`Path(__file__).resolve().parents[1]`), а не CWD-relative: тело класса
  исполняется на collection, поэтому запуск pytest не из корня убивал файл
  целиком. Добавлен пин обратного направления для LOCKED-правила «edit distance
  ≤ 1»: «ладно» → `single_token_unlisted`.
...
- **Metrics / Tests** — `create_instrumentator()` принимает необязательный
  `registry`. Прод-путь (`registry=None`) не изменился, включая синглтон;
  явный registry синглтон обходит и отдаёт свежий инструментатор. Это нужно,
  чтобы guard выше собирал приложение прод-фабрикой, а не копией её настроек:
  копия — ровно тот механизм, из-за которого BUG-089 и дожил до прода
  (проверено мутацией — с возвращённым `metric_subsystem="http"` тесты падают,
  на копии проходили).
- **Metrics / Tests** — бакеты latency вынесены в модульную константу
  `HTTP_LATENCY_LOWR_BUCKETS` (`tg_parser/api/metrics.py`): фабрика передаёт её
  в `metrics.default()`, а guard импортирует её и сверяет с ней ВЕСЬ набор `le`,
  вытащенный из тела `/metrics` (сравнение по float — форматирование
  `floatToGoString` не контракт; бакет `+Inf` игнорируется). Раньше
  проверялась одна граница `le="0.01"`, поэтому удаление хвоста `30.0, 60.0`
  оставляло набор зелёным — а это ровно то решение из пункта выше, которое
  держит `histogram_quantile(0.99)` подальше от `+Inf` на RAG-ручках. Одного
  равенства мало по построению: обе его стороны читаются из фабрики, и при
  обрезке тупля ожидание съезжает вместе с реальностью. Поэтому потолок
  закреплён отдельно — против соседней `LLM_REQUEST_DURATION_SECONDS`, откуда
  60 s и взялись: `max(exposed) >= max(конечных границ LLM-гистограммы)`. Ни
  одно число в тест руками не переписано. Проверено мутацией: удаление
  `30.0, 60.0` теперь красное (`assert 10.0 >= 60.0`), до правки — зелёное.
- **Tests** — порог против вакуумного прохода в сверке имён теперь на КАЖДЫЙ
  файл, а не на пару: `assert len(checked) >= 2` выполнялся одним
  `system.json` (он даёт 2 проверенных имени, `alerts.yml` — одно), так что
  `alerts.yml` можно было опустошить или сломать на нём регулярку и ничего бы
  не упало. Теперь каждый конфиг потребителя обязан дать хотя бы одно
  проверенное `tg_parser_http_*`, а сообщение об ошибке печатает счётчики по
  файлам. Проверено мутацией: вычистка всех упоминаний `tg_parser_http_*` из
  `alerts.yml` — красная.
...
- **Infra** — Prometheus bumped `v2.53.0` → `v3.13.2` (`docker-compose.yml`).
  Смена LTS-линии, а не шаг по ней: `v2.53` снят с поддержки, `v3.5` LTS
  закончился 2026-07-31, поэтому взят текущий LTS `v3.13` (поддержка до
  2027-07-31). Конфиг, правила и три флага запуска (`--config.file`,
  `--storage.tsdb.retention.time=30d`, `--web.enable-lifecycle`) совместимы
  без правок. Три риска v3 проверены эмпирически и не подтвердились: все
  `/metrics` отдают `text/plain; version=1.0.0` (строгая проверка
  Content-Type проходит, `fallback_scrape_protocol` не нужен); в значениях
  меток `status` / `model` / `stage` нет переводов строки (новое поведение
  `.` в regex безвредно); `le` / `quantile` / `holt_winters` в 53 выражениях
  дашбордов не встречаются (канонизация float-меток не задевает).
- **Alerting** — юнит-тесты правил (`docker/prometheus/alerts_test.yml`)
  проходят только на v3: v2.53 рендерит в аннотацию `0.30000000000000004`
  там, где тест ждёт `0.3`. Правила изначально писались и валидировались
  локальным promtool v3.x, а прод отставал; `promtool` в CI не гоняется,
  поэтому расхождение не всплывало. Апгрейд приводит прод в соответствие с
  тем, против чего правила разрабатывались.

**Откат.** Формат TSDB v3 читается только Prometheus 2.55+. После первой
записи возврат на 2.53 возможен только восстановлением тома из бэкапа,
снятого до переключения (том ~55 MB, retention 30d — теряется история
мониторинга, пользовательских данных там нет).
