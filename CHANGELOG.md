...
### BUG-097 (a) / R10: дедупликация перестала считаться отказом (2026-08-13)

**У дубликата теперь свой исход.** `_batch_post_llm_dedup` в [`pipeline.py`](tg_parser/processing/pipeline.py) считает документы, которые прошли обработку и были отброшены post-LLM дедупом (`dedup_db_duplicate` / `dedup_within_batch_duplicate`); `deduplicated_count` выносит это в `process_stats`, и `failed_count` считается как `total − processed − skipped − deduplicated`. Счётчик встал рядом с двумя прецедентами той же переклассификации — `_batch_cooldown_skipped` (BUG-067 B2b) и `_batch_pre_llm_deferred` (S3 O-2), — но **не** переехал в их ведро: те документы до обработки не доходили, а дубликат обработку прошёл, и «skipped» для него был бы ложью другого рода. До этого он попадал в `failed` по остаточному принципу: не `processed`, не `skipped` — значит арифметически отказ, хотя так его никто не называл.

**Знаменатель B1 — решение, а не наследство.** `attempted` по-прежнему включает дедуплицированные документы: они были взяты в обработку, и обработка **удалась** — отбросили их уже по пути в хранилище. Правило единое для всех трёх переклассификаций — «attempted = документ был взят в обработку в этом тике» (`len(to_process)`), и по нему же cooldown-skipped и pre-LLM deferred исключены из **обеих** частей отношения. `fail_ratio` тем самым читается как «доля попыток обработки, которые упали». Порог `scheduler_degraded_failure_ratio` не тронут: при честном числителе 0.5 работает как задумано.

**Видимость нового исхода — отдельное решение, а не побочный эффект.** INFO-строка `source_tick_deduplicated` в тике (там, где жила ложная тревога), `total_deduplicated` в сводке тика, `deduplicated_count` в `source_attempts.details_json.pipeline_stats`, в CLI и в результате API-джобы; метрика `tg_dedup_duplicates_detected_total` существовала и раньше. В `get_pipeline_status` поля **нет** намеренно: эта поверхность читает текущее состояние из таблицы `sources`, и счётчик дубликатов потребовал бы новой колонки, чей единственный потребитель — стоимостный вопрос R11, на который метрика уже отвечает, причём с историей. Молча прятать тоже нельзя: дедуп на четырёхзначных числах — это сигнал для R11, а не шум.

**Два инварианта, которых в наборе не было.** [`tests/test_bug097_dedup_classification.py`](tests/test_bug097_dedup_classification.py) — 9 тестов, из них 7 красных на коде до фикса, и по правильным причинам: `failed_count` возвращал 3 на тике без единого отказа, а планировщик писал `fail_ratio=1.0` — ту же строку, что видна на проде. Инвариант про `degraded` прогоняет **настоящий** `run_processing` и подаёт его вывод планировщику: собранный руками dict сделал бы тест зелёным по построению, потому что вся правка живёт выше планировщика. Плюс страховка от перекоррекции (два настоящих отказа из трёх попыток по-прежнему дают `degraded`) и проверка, что дедуп отбрасывает те же документы. Прогоны: default `3848 passed`, PR standard (`TEST_POSTGRES=1`) `4260 passed, 22 skipped, 2 deselected`.

**Накопленный `fail_count` сбрасывать не нужно — находка, отменившая операционный пункт промпта.** `record_attempt(success=True)` делает `SET fail_count = 0, last_error = NULL`, поэтому после деплоя первый же тик без настоящих отказов обнулит и счётчик (до 1076), и текст «processed 0 of N». Стартовый промпт R10 §3.4 предполагал обратное и предлагал готовить команду сброса; она не понадобится.

**Для половины (b) прочитан ответ, но порядок не менялся.** Предусловие R11 («почему документы минуют pre-LLM dedup из S3») снято чтением кода: pre-LLM проверка хэширует **сырой** текст, post-LLM — **вывод LLM**, и расходятся они по трём механизмам. Нормализация LLM (разный сырой текст, одинаковый чистый) переносом проверки не лечится вообще: `text_clean` до вызова не существует. Legacy-строки без `raw_content_hash` (S3 приехал в июле, на проде 45 750 документов) лечатся бэкфиллом, а не порядком. Media-only и пустые сообщения не имеют сырого хэша по построению, а их синтетический `text_clean` массово коллизирует — но собираются они **без** вызова LLM, то есть часть отброшенных дубликатов не стоила ни токена. Поэтому R11 начинается с раскладки 81 дубликата по этим трём долям: без неё оценка экономии — догадка. Записано в [BUG-097](docs/notes/BUG_LOG.md); порядок вызова дедупа не тронут, как требовало ограничение сессии.

**База для сравнения снята на проде 2026-08-13 (read-only).** Сначала здесь стояло «пересчёт не выполнен, прод в локальном режиме недоступен» — неверно: `dev_doctor` пробует `ssh prod` изнутри агентской песочницы и печатает `MISS`, хотя доступ есть (правило записано в [`AGENTS.md`](AGENTS.md), «Песочница»). Замер: 8 из 14 источников с `fail_count` = 1079 / 1005 / 1004 / 945 / 866 / 642 / 473 / 144, все `active` и с `last_success_at` этого же часа. Проверять после деплоя: источник, у которого в тике были только дубликаты, не в `degraded`, `fail_count` = 0, `last_error` = NULL.

**Задеплоено 2026-08-13 по GO владельца** (merge-коммит `581edd1`; прод `4da7025` → `581edd1`). Пересоздан **только** контейнер `tg_parser` — носитель планировщика и processing-пути; `tg_parser_mcp` не тронут намеренно, чтобы не стереть окно логов, на которое опирается сессия R1, а `tg_bot` правка не касается. Точка отката — образ `94b713377d91` под тегом `tg_parser:pre-pr416-2026-08-13`; ни миграции, ни правки `.env` фикс не требует. Ложная тревога снялась **на первом же тике** (19:27–19:28 UTC) и целиком: 8 источников с `fail_count` 1080 / 1006 / 1005 / 946 / 867 / 643 / 474 / 145 и `last_error` «processed 0 of N attempted» → **0 у всех 14**, `last_error` = NULL, все `active`. Ручных команд не выполнялось — сработала предсказанная ветка `record_attempt(success=True)`. Новый исход сошёлся в трёх независимых местах канал в канал: 8 INFO-строк `source_tick_deduplicated`, `deduplicated_count` = 8 / 5 / 4 / 3 / 2 / 2 / 2 / 1 в `source_attempts` при `failed_count: 0` и `success=true`, и те же числа в `tg_dedup_duplicates_detected_total`; `source_tick_degraded` — 0 вхождений. Здоровый путь не задет (`medportal_rfed`: `processed_count: 1` в том же тике). Перевернулся и агрегат, который был симптомом: за 41 ч до фикса 320 попыток `success=false` против 240 успешных, после — **28 попыток, все `success=true`**. **Не проверено на месте:** что настоящий отказ по-прежнему регистрируется — в окне не было ни одного реального сбоя (`processing_failed` / persist-ошибки: 0), изготавливать его намеренно не стали, и страховкой служит тест, а не наблюдение. Процедура и фактические числа — [`docs/runbooks/BUG097_R10_DEPLOY_AND_WATCH.md`](docs/runbooks/BUG097_R10_DEPLOY_AND_WATCH.md).

**Деплой вскрыл дефект в формулировке собственной проверки.** «Дождаться следующего тика, они идут примерно раз в 30–40 минут» неверно дважды: интервал ровно 3600 с, а `incremental_pipeline` — **interval**-задача APScheduler, а не cron, поэтому её фаза привязана к старту процесса. До деплоя тики шли в :00–:01, потому что прежний контейнер поднялся около 14:00 UTC; пересозданный в 18:27:05 дал первый тик в **19:27**, а не в 19:00. То есть пересоздание контейнера сдвигает сетку тиков, и опорной точкой ожидания должна быть строка `Background scheduler started` в логах, а не `last_success_at` предыдущих часов. Записано в §3 runbook'а. Отдельно там же зафиксировано, что мгновенный запрос `tg_dedup_duplicates_detected_total` сразу после пересоздания **пуст** и это норма (процессный счётчик с метками не имеет серий до первого инкремента), поэтому базу для сравнения надо брать из истории Prometheus параметром `&time=`, а не из живого запроса.

**Тот же замер переформулировал половину (b), которую R10 не трогала.** За 41 час попытки, помеченные отказом, обработали 1108 документов и сожгли **1 495 164 токена** против 10 документов и 20 416 токенов на успешных — ≈99 % токенов стадии processing. Причина не в объёме дубликатов, а в том, что расход **бессрочный**: в сохранившемся окне логов 81 событие `dedup_db_duplicate` приходится на **27 уникальных** документов, каждый ровно трижды, ни одного однократного. Отброшенный документ не сохраняется, а работа выбирается через `NOT EXISTS` в `processed_documents` — значит следующий тик снова видит его необработанным и снова платит за summary. Поэтому первым шагом R11 становится **запись факта отбраковки**, а перенос проверки перед LLM — вторым: он экономит только первый вызов на документ. Подробности и числа — в [BUG-097](docs/notes/BUG_LOG.md).

### BUG-095 / R8: доставка инстант-алертов F11 переехала в бот-процесс (2026-08-13)

**Форма B реализована.** `watchlist_instant_flush` — interval-задача (по умолчанию **300 с**) в процессе бота — выбирает `notified=false` матчи активных `instant`-интересов и шлёт их **тем же** `_send_group`, что и батч-путь: отправка, ретраи, обработка блокировки чата и watermark остаются в одной реализации. Матчер как жил в `tg_parser`, так и живёт; он теперь только записывает матч. Регистрация — `bot/main.py` рядом с `_register_watchlist_batch_flush`; в `setup_default_tasks` запрещена, и это удерживается тестом, который разбирает функцию AST'ом, а не комментарием (комментарий там был — и всё равно второй путь остался сломанным).

**Watermark, а не дисциплина оператора.** `list_unnotified_for_interests` получил границы `since` / `before`; flush ходит только за `created_at >= watermark`. Без watermark'а он **не запускается вовсе** (`skipped_reason="no_watermark"`): при отсутствующей границе «доставить всё» — это ровно тот залп двухмесячного бэклога, ради предотвращения которого граница и заведена, поэтому отказ громкий, а не мягкая деградация. Источник границы — `WATCHLIST_INSTANT_FLUSH_CUTOFF` либо момент регистрации задачи.

**Молчание закрыто с двух сторон.** `check_interests` без бота теперь логирует `watchlist.instant_delivery_deferred reason="no_bot"` — как батч-сосед с 2026-06-11. Плюс гейдж `tg_watchlist_undelivered_matches` (матчи, оставшиеся недоставленными дольше одного интервала) и алерт `WatchlistMatchesUndelivered` (`> 0`, `for: 1h`). Существующий `WatchlistDeliveryErrors` этот класс поймать не мог по устройству: он меряет неудачные отправки, а их не было — попыток не было.

**Бэклог: 93, а не 76.** Пересчёт read-only на проде 2026-08-13: **93 матча, 14 интересов, все в одном чате**, все `instant` и активные (снимок 08-12 протух, как и предупреждал промпт). Разбор — `scripts/watchlist_backlog_summary.py`: одна сводка на чат («интерес → сколько пропущено, за какой период» + подсказка про `get_watchlist_matches`), не список постов. Идемпотентность взята из того же `notified`-watermark'а, а не из таблицы-маркера: рабочее множество — строки `notified=false`, успешная отправка их переключает, поэтому второй прогон отправляет ноль.

**Контракт `instant` переписан явно.** [ADR-0014](docs/adr/0014-watchlist-batch-silent-delivery.md) дополнен разделом «Instant delivery topology»: `instant` официально означает «в пределах одного интервала flush'а», а не «синхронно с тиком». Заодно исправлена Fork 1 — она называла местом регистрации `setup_default_tasks`, что **неверно с 2026-06-11** (`52a2ea8`); тот самый дрейф, из-за которого принятый ADR подтверждал бы новой задаче неправильный процесс. **Находка:** ADR-0010 инстант-push вообще не описывает — исходный контракт жил в `FUTURE_FEATURES` / `USER_GUIDE` / `README`, а не в ADR, поэтому правились они (закрыт отложенный пункт аудита #2 §6 п.1).

**Прогоны:** default `3834 passed`, PR standard (`TEST_POSTGRES=1`) `4246 passed`; `promtool check rules` + `test rules` зелёные. Red/green: на коде до фикса `9 failed, 3 passed`, после — `36 passed` (три зелёных на сломанном коде — это и есть формулировка причины).

**Задеплоено 2026-08-13 по GO владельца** (merge-коммит `4da7025`). Watermark закреплён в прод-`.env` как `WATCHLIST_INSTANT_FLUSH_CUTOFF=2026-08-13T00:00:00Z` — то же значение ушло границей в разбор бэклога, иначе между двумя половинами партиции остались бы щель или нахлёст. Скрипт отправил **одну** сводку на 93 матча в один чат, повторный прогон — ноль; `notified=false` схлопнулся в 0 из 449 строк. Задача зарегистрирована с `interval_seconds=300`, тики идут (0.08 с и 0.03 с), гейдж `tg_watchlist_undelivered_matches` = 0 в обеих сериях (`job=tg_parser_bot` на `tg_bot:8081` и `job=tg_parser_api`). BUG-095 → `resolved`. Единственная проверка, не закрытая на месте, — доставка **нового** матча: с момента деплоя ни одного не появилось, механизм подтверждён тиками.

**Деплой вскрыл два дефекта в собственном runbook'е, оба исправлены здесь же.** Первое: `docker exec tg_parser_bot python scripts/…` не мог работать никогда — Dockerfile копирует в образ только `tg_parser/`, `prompts/` и `migrations/`, каталога `scripts/` там нет. Runbook переписан на `docker cp` вместо правки Dockerfile осознанно: в `scripts/` лежит весь ops-инструментарий, включая `onboard_test_users.py`, и прод-образу он не нужен. Второе: приехавший в этом же PR алерт **не существовал** после деплоя — файл `alerts.yml` обновился с `git pull`, но Prometheus конфиг не перечитывал, и шага перезагрузки в runbook'е не было. Добавлен, через `--web.enable-lifecycle`.

**Отдельно: последний коммит сессии R8 ронял сборку тестов.** `tests/test_bug095_backlog_script.py` импортировал `from scripts.watchlist_backlog_summary import _parse_before` без корня репозитория в `sys.path` — `ModuleNotFoundError: No module named 'scripts'` на этапе collection, то есть **весь** сьют из 4270 тестов не выполнялся, а не один падал. Исправлено прелюдией из `tests/test_onboard_test_users.py` (коммит `4186626`). Перед мержем на смерженном `main`: ruff чисто, PR standard `4251 passed, 22 skipped`.

### Замер для R1 снят до деплоя R8; окно логов оказалось скользящим, а не растущим (2026-08-13)

**Снято по решению владельца, пока логи живы.** `mcp.auth.static_fallback_used` = **0** за всё сохранившееся окно `tg_parser_mcp` — **2026-08-12 18:08:02 → 2026-08-13 11:19:00 UTC**. Рядом: `identity_missing` = 0 (fail-loud ветка не срабатывала), `mcp.tool.start` = 2 (аутентифицированных tool-вызовов в окне всего два), `mcp.request.received` = 13 812 — почти целиком неаутентифицированные scrape'ы `/health` и `/metrics`, поэтому знаменателем для auth-событий это число не является. `identity_resolved` = 0 ничего не доказывает: это `debug`, а DEBUG выключен.

**Ошибка в моём же вчерашнем совете, обнаруженная замером.** Я говорил, что за ночь окно вырастет с семи часов до суток. Оно не выросло — оно **сдвинулось**: контейнер живёт 22 часа, в логе сохранилось 17, первые пять часов съедены ротацией (`json-file`, `max-size=10m` × `max-file=3`; 162 263 строки в окне). Окно **скользит**, а не растёт, поэтому «подождать и посмотреть» не работает в принципе. Представительную выборку может дать только Prometheus-счётчик на событие деградации — это правка кода внутри сессии R1, и она добавлена в её scope как обязательный deliverable.

**Что замер разрешает и что нет.** Разрешает считать, что неожиданного статического клиента на проде нет — ноль событий в двух независимых окнах (второе: 08-12 13:11–18:55, 10 tool-вызовов). **Не** разрешает удалять legacy-ветку: суммарная аутентифицированная выборка ~12 вызовов. Форма фикса от замера и не зависит — разделение «UUID не резолвился → отказ» / «не-UUID → legacy admin» корректно при любом исходе. Роль замера — исключить неожиданность, а не выдать лицензию на удаление.

Из ограничений промпта R8 снят пункт «снять замер до деплоя»; вместо него записано правило на будущее: перед любым деплоем проверять, не висит ли на логах чей-то незакрытый замер.

**Deploy (docs-only):** три файла `docs/notes/**` плюс этот `CHANGELOG.md`.

### BUG-095: выбрана форма фикса B, R7 растворилась, промпт стал R8 (2026-08-13)

**Решение владельца: форма B** — инстант-матчи остаются `notified=false`, доставляет их **бот-процесс** частым flush'ем, зеркалящим уже работающий `flush_batch` (`notify_mode == INSTANT` вместо `BATCH`, общий `_send_group`). Отклонены: A (send-only `Bot` в `tg_parser` — новый секрет в периметре плюс ловушка compose allow-list), C (HTTP-поверхности в боте нет), D (рвёт связку хука с тиком). Цена — «instant» становится «до интервала»; при интервале в минуты это неотличимо от сегодняшнего состояния, где доставки нет вовсе.

**R7 растворилась.** Она существовала ради двух выходов, и оба получены вне сессии: причина установлена при подготовке промпта, форма фикса выбрана владельцем. Осталось подтверждение red/green — первый шаг любого фикса, не отдельная сессия. Это то же правило, по которому раньше растворилось дробление R4a/R4b: **граница сессии стоит там, где стоит неизвестность, и снимается вместе с ней.** BUG-095 теперь закрывается одной сессией **R8**; нумерация не сдвигается, промпт переименован в `..._R8_...`.

**⚠️ Найдена ловушка формы B — и она меняет ранее записанный вывод.** `list_unnotified_for_interests` селектит **все** `notified=false` без ограничения по дате, а `flush_batch` фильтрует лишь по `is_active` и `notify_mode`. Значит новый инстант-flush на первом же запуске вышлет все 76 накопленных матчей — сам фикс устроил бы спам, которого избегает решение по бэклогу. Ранее было записано «бэклог блокирует метрику, а не фикс»: при форме B он блокирует и включение flush'а. Закрывается **watermark'ом в селекторе** (`created_at >= момент включения`), а не порядком действий оператора — watermark робастен к повторному деплою и откату, порядок нет.

**R8 заодно закрывает отложенный пункт аудита #2** (§6 п.1): пять живых документов обещают instant push; после формы B это не оговорка, а обновление реального поведения.

**Deploy (docs-only):** три файла `docs/notes/**` плюс этот `CHANGELOG.md`. Сама сессия R8 будет **не** docs-only — там код и расписание в бот-процессе.

### BUG-095: root cause установлен; START_PROMPT сессии R7 (2026-08-13)

**Причина недоставки алертов watchlist найдена — из кода и read-only прогона на проде, без единой правки.** Инстант-матчер работает в процессе `tg_parser`, где нет и не может быть `Bot`, а `check_interests` при `bot is None` **молча** пропускает доставку. Пять звеньев, каждое проверено:

1. `run_watchlist_check_for_channel` передаёт `bot=get_bot()` **без guard'а на `None`** (`scheduler_service.py:1302–1306`);
2. тик идёт в `tg_parser`: **294** строки `watchlist_check` в его логах против **0** в `tg_parser_bot`; обратно `watchlist_batch_flush` — **6** в боте против **0** в `tg_parser`;
3. `get_bot()` — **process-local** синглтон (`bot/runtime.py`), заполняется только `set_bot()` при старте бота; в `tg_parser` его не вызывает никто, и `TELEGRAM_BOT_TOKEN` в этот контейнер не передан;
4. `watchlist_service.py:1336` — `if bot is not None and inserted:`, то есть ни лога, ни `skipped_reason`, ни метрики;
5. следов попытки отправки в `tg_parser` нет вовсе: `watchlist.notify*` = 0, `watchlist.*sent` = 0.

**Почему это молчало два месяца — отдельный дефект, а не следствие.** Батч-сосед в том же файле guard'ит и **говорит**: `logger.info("watchlist_batch_flush_skipped", reason="no_bot")`. Инстант-путь того же не делает. Система знала, что доставлять некуда, и не сказала. Правка этой асимметрии входит в R8 независимо от выбранной формы фикса.

**Почему контракт `get_bot()` здесь не работает.** Его докстринг обещает «skip delivery and *not* advance the digest cursor» — безопасно для дайджестов, где курсор не двигается и следующий тик доставит. У инстант-watchlist **нет курсора и нет повторной попытки**: матч пишется один раз с `notified=false`, и к нему никто не возвращается. Инстант-путь взял контракт, корректный только для путей с retry.

**Граница 2026-06-15 переоценена:** она остаётся фактом, но перестала быть главной зацепкой — причина найдена из топологии процессов, а не из диффа того дня. Кандидат в объяснение самой даты — коммит `221f75f` (2026-06-11, [ADR-0014](docs/adr/0014-watchlist-batch-silent-delivery.md)), извлёкший общий `_send_group` с заявлением «no change to the instant path». Логи июня недоступны навсегда (ротация + пересоздание контейнера стирает каталог логов), поэтому дата проверяется git-историей, а не чтением логов.

**Сессия R7 переписана** — из диагностической в короткую сессию подтверждения и выбора формы фикса (четыре варианта, рекомендация — доставка инстант-матчей из бот-процесса по уже работающей batch-машинерии). Стартовый промпт: [`START_PROMPT_FIX_BUG095_WATCHLIST_DELIVERY_R8_2026-08-13.md`](docs/notes/START_PROMPT_FIX_BUG095_WATCHLIST_DELIVERY_R8_2026-08-13.md).

**Заодно:** в плане исправлений записано, что его охват шире имени — файл назван по ревью bot + MCP, но покрывает всю очередь, включая BUG-094…098. Имя не меняется: на него уже ссылаются 11 записей `BUG_LOG`, `DECISION…SESSIONS` §6 и две записи `CHANGELOG`.

**Deploy (docs-only):** три файла `docs/notes/**` плюс этот `CHANGELOG.md`; кода приложения нет.

### Карта fix-сессий R1…R12 и четыре решения владельца по открытым багам (2026-08-13)

**Что решено.** Четыре вопроса, висевших в записях `BUG_LOG` как «owner decision», закрыты — и два из них изменили саму карту сессий, а не только scope внутри:

| Вопрос | Решение | Следствие |
|---|---|---|
| BUG-096 — как свести writer и reader экспорта | **(b)** HTTP-dispatch, как у `trigger_*` | дробление `R4a`/`R4b` **растворилось** → одна сессия `R4`. Оно держалось только на открытом выборе |
| BUG-094 — частичный update или отдельный `update_channel` | **(a)** частичный update | сессия `R9`; **(b)** снят с очереди фиксов в parking-lot Wave 3 — он правит принятый ADR-0009 и оставляет без дома оживление soft-deleted канала |
| BUG-095 — судьба 76 недоставленных матчей | **гибрид**: пометить обработанными + **одна** сводка «интерес → сколько пропущено, за какой период» | разбор вошёл в `R8` между фиксом и метрикой — в этом порядке, а не в любом |
| BUG-102 — дата снятия legacy-ключей | **окно депрекации не нужно**, снимаем в `R3` одним шагом | из scope `R3` ушёл переходный период; основание померено, а не оценено (0 читателей в коде, 4 строки в 2 тестах) |

**Поправка, найденная при подготовке решений — важнее самих решений.** План исходил из того, что решение по бэклогу BUG-095 блокирует **деплой фикса**. Это неверно: instant-путь (`notify`) получает группы матчей текущего тика от `check_interests` и ничего не селектит из БД, а `notified=false` выбирает только batch-путь (`list_unnotified_for_interests`, и только для активных интересов с `notify_mode='batch'` — их сейчас ноль). Значит починка instant-пути go-forward и бэклог не высыпет. Решение блокирует **метрику**: гейдж «недоставленные старше одного тика» родился бы красным на 76 и учил бы оператора игнорировать алерт — ровно тот механизм, из-за которого поломка жила два месяца. Отдельная ловушка до разбора бэклога: перевод любого интереса в `notify_mode='batch'` отдаст его накопленные матчи пачкой на следующем суточном flush'е.

**Карта сессий.** [`PLAN_REMEDIATION_BOT_MCP_2026-08-12.md`](docs/notes/PLAN_REMEDIATION_BOT_MCP_2026-08-12.md) расширен с шести сессий до **двенадцати** — `R1…R6` по находкам ревью, `R7…R12` по багам, открытым до него, — с принципами декомпозиции (§1c) и порядком (§4):

```
R7 ──► R8 ──► R1 ──► R2 ──► R9 ──► R4 ──► R3 ──► R5
(095   (095   (099)  (100,  (094)  (096)  (102,  (103)
 диаг) фикс)         101)                  098a)
  └── параллельный трек:  R10 · R11 (097 a/b) · R12 (098b) · R6 (104)
```

Границы сессий определяет не тематика, а расположение неизвестности: BUG-095 и BUG-097 дробятся, потому что внутри них открытый вопрос; BUG-096 перестал дробиться, как только вопрос закрылся. Ни один из шести багов, заведённых **по** ревью, дробить не нужно — ревью успело ответить на вопросы, поэтому `BUG-099…104` приходят готовыми к сессии.

**Первой идёт `R7`** — диагностика BUG-095, единственной поломки, которую видит живой пользователь. Она не требует ни одного открытого решения.

**Deploy (docs-only).** Четыре файла `docs/notes/**` плюс этот `CHANGELOG.md`; кода приложения нет, поэтому прод подтягивается `git pull` без пересборки и без пересоздания контейнеров.

### Заведены BUG-099…104 и утверждена очередь исправлений по ревью bot + MCP (2026-08-13)

**Что записано.** Двенадцать находок `F-01…F-12` из
[`CODE_REVIEW_BOT_MCP_2026-08-12.md`](docs/notes/CODE_REVIEW_BOT_MCP_2026-08-12.md)
превращены в шесть записей `BUG_LOG` — **BUG-099…104**, — плюс два дополнения к
уже открытым: `F-03` → BUG-096 (назван механизм: MCP импортирует приватную
`_run_export_job` из `api.routes.export` и исполняет джобу API в своём процессе;
писателей одного пути три, не два), `F-07` → BUG-098 (половина (a) — это решение
о типе возврата, а не добавление поля). Открытых записей стало **12 из 104**;
следующий свободный id — `BUG-105` (выведен командой, не назначен на глаз).

**Гранулярность записей — «одна запись = одна fix-сессия», а не «одна находка =
одна запись»:** так каждая закрывается одним PR и одним прогоном, а не остаётся
половиной себя открытой. R1 → BUG-099, R2 → BUG-100 + BUG-101, R3 → BUG-102
(+ половина (a) BUG-098), R5 → BUG-103, R6 → BUG-104; R4 своей записи не
получила — её содержание уехало в BUG-096.

**Очередь утверждена владельцем и зафиксирована в одном месте** —
[`PLAN_REMEDIATION_BOT_MCP_2026-08-12.md`](docs/notes/PLAN_REMEDIATION_BOT_MCP_2026-08-12.md)
§4, статус документа `proposed` → **`accepted`**:
`BUG-095 → R1 → R2 → BUG-094 → R4a → R4b → R3 → R5`, параллельным треком
`BUG-097`, `BUG-098(b)`, `R6`. Блок «что открыто» в `BUG_LOG` несёт колонку
«сессия» как ярлык для сшивки и **не воспроизводит** порядок: один статус — одно
место, один порядок — одно место.

**Шаг 0 (внешние валидаторы Wave 1.5) — вердикт владельца 2026-08-13: «запуск»,
подключение в работе.** Записано в
[`DECISION_AUDIT_AND_STRATEGY_SESSIONS_2026-08-12.md`](docs/notes/DECISION_AUDIT_AND_STRATEGY_SESSIONS_2026-08-12.md)
§0 с операционным следствием: сессия #4 остаётся закрытой гейтом до **исхода**
(валидаторы дали сигналы либо попытка закрыта отказом) — промежуточное «в работе»
входом не является, иначе #4 снова получит `0/0/0` и будет трактовать отсутствие
данных как измерение. Очередь исправлений от этого не зависит. Второй открытый
пункт остаётся открытым: одна дата Forced DP вместо нынешней «~2026-09-01».

**Deploy (docs-only).** Три файла `docs/notes/**` плюс этот `CHANGELOG.md`; кода
приложения нет, поэтому прод подтягивается `git pull` без пересборки и без
пересоздания контейнеров — критерий тот же, что в записи ниже. SHA merge-коммита
здесь намеренно не зашит: состояние прода читается командой
`ssh prod 'cd /home/user/TG_parser && git log -1'`, а не из этой записи.

### Deploy (docs-only) — прод на `acf9589`: код-ревью bot + MCP (PR #402), план fix-сессий (PR #403), запись прошлого деплоя (PR #401) — 2026-08-12 20:2x UTC

**Что на проде.** `f005f93 → acf9589` одним fast-forward'ом. В окне — три PR,
влитые подряд в том же порядке: [#401](https://github.com/AlexEfimov/TG_parser/pull/401)
(запись деплоя, который привёз #399), [#402](https://github.com/AlexEfimov/TG_parser/pull/402)
(артефакт код-ревью сессии #3 — 12 находок `F-01…F-12`) и
[#403](https://github.com/AlexEfimov/TG_parser/pull/403) (план remediation-сессий
R1–R6). Состав окна: `git diff --stat f005f93..acf9589` — **7 файлов, 572
вставки**, из них шесть markdown и один CI-конфиг.

**Сборка и пересоздание не требовались, и это проверено, а не предположено.**
Кода приложения в окне нет вовсе: пять файлов `docs/notes/**`, один
`CHANGELOG.md` и `.github/workflows/markdown-link-check-config.json` — последний
читает только GitHub Actions, не рантайм. Критерий тот же, что в записи ниже:
единственное, что приложение грузит с диска, — `prompts/*.yaml`. Проверка после
pull'а: `StartedAt` контейнеров `tg_parser` / `tg_parser_mcp` / `tg_parser_bot`
остался `2026-08-12T13:11:15Z` / `13:11:15Z` / `13:11:35Z` — то есть тот же, что
до pull'а, — все три `healthy`, api `/health` → 200.

**Содержимое сверено по файлам, а не по SHA.** На проде:
`CODE_REVIEW_BOT_MCP_2026-08-12.md` — 347 строк и 12 заголовков `### F-`;
`PLAN_REMEDIATION_BOT_MCP_2026-08-12.md` — 176 строк и 7 секций `### R`
(R1, R2, R4a, R4b, R3, R5, R6); `PLAN_*` и `START_PROMPT_*` сессии #3 лежат в
`docs/notes/archive/` (2 файла) и отсутствуют на верхнем уровне (0);
в конфиге линк-чекера 8 `ignorePatterns`, второй — `^https?://github\.com`.

**Единственное функциональное изменение окна — в CI, не в проде.** Джоба
`Lint Documentation` падала три прогона подряд на 16–18 «мёртвых» ссылках со
`Status: 0`, из которых ни одна цель не была сломана: в корпусе 417 абсолютных
ссылок на собственные PR/issue/commit этого репозитория, чекер ходит анонимно,
github.com его душит. Ретриггер (тот же приём, что `7692b51` тем же утром)
перестал помогать — это и есть признак, что джоба отчитывалась о сети. Добавлен
`^https?://github\.com` в `ignorePatterns` рядом с `cursor.com/automations`,
который в том же файле уже пропускается по той же причине. Цена названа в
комментарии конфига: опечатка в github.com-URL больше не ловится CI;
относительные ссылки эта джоба не проверяла никогда.

> **Про эту запись.** Она добавлена сразу после деплоя, а не задним числом, —
> ровно то, чего не хватало шести молчаливым pull'ам из записи ниже. Сама она
> приедет на прод следующим pull'ом; состояние прода по-прежнему честнее брать
> командой (`ssh prod 'cd /home/user/TG_parser && git log -1'`), а не из
> последней записи здесь.
