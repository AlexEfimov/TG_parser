...
**Деплой не выполнен:** merge, пересоздание контейнеров и один прогон скрипта на проде ждут явного GO владельца. Процедура — [`docs/runbooks/BUG095_DEPLOY_AND_WATCH.md`](docs/runbooks/BUG095_DEPLOY_AND_WATCH.md). BUG-095 держит статус `in-progress`, не `resolved`.
