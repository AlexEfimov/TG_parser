...
> `cursor/vps-watch-incident-ingress-d032` for the operator to forward. Title /
...
>   filed by seven prior ingress runs:
...
>   * branch `cursor/vps-watch-incident-ingress-b490` — trigger `2026-06-12T01:44Z`
>
>   This is the **same payload re-delivered** at automation trigger time
>   `2026-06-14T08:04Z`. Per the agent contract `one payload = at most one
...
>   The persistence of this group across ~12 days strongly indicates a
...
* `endsAt` = `0001-01-01T00:00:00Z` — *unresolved* (open-ended ⇒ still holding
  at automation trigger time `2026-06-14T08:04Z`, ~11.6 days after `startsAt`).
...
4. **Watch-window posture (strong signal this run).** Nominal close was
   `2026-05-25T10:50Z`; trigger time is `2026-06-14T08:04Z` — ~20 days past
   nominal close, Wave 1 already declared closed. The same group with identical
   fingerprints has now re-delivered **eight** times over ~8 days
...
6. **Dedupe note.** Fingerprints `b5a3e0633aff0a2f`, `ee79af1ca55d3c5c`,
   `42fe0f4e137067ac` observed by this ingress run (eighth re-delivery of the
   group already seen on branches `...-8551`, `...-c056`, `...-7045`,
   `...-de76`, `...-e520`, `...-8a68`, `...-b490`). Per agent contract (`one
   payload = at most one issue`) this single artifact is the only deliverable
   for this run; no further follow-ups unless Grafana re-triggers with
   different fingerprints or the operator explicitly retriggers.
...
* **Step 3 (filing):** title `[5xx] DatasourceError (Wave 1 step 4 VPS watch)`;
  ...
  artifact on branch `cursor/vps-watch-incident-ingress-d032` (cloud-agent
