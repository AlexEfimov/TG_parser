...
> 💡 **Совет:** Для экономии используйте модель `gpt-4o-mini` (default). Она дешевле и достаточна для большинства задач.
...
# Модель LLM (опционально, default: gpt-4o-mini)
LLM_MODEL=gpt-4o-mini

# Для OpenAI-compatible провайдеров (опционально)
...
Обрабатывает raw сообщения через OpenAI LLM: очистка текста, саммари, извлечение тем и сущностей.

```bash
python -m tg_parser.cli process --channel <channel-id>
```
...
- `--dry-run` — режим проверки (пока не реализован)
...
# Принудительная переобработка всех
