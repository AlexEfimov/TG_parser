...
**Accepted (2026-08-12).** Первый on-path тик отработал в проде — условие приёмки выполнено; доказательства ниже.

### Update 2026-08-12 — первый prod-запуск состоялся, статус переведён в `Accepted`

Cron отработал `2026-08-12 03:30:00 UTC` — все три ожидавшихся сигнала совпали с записанными заранее:

| Сигнал | Ожидалось | Факт |
|---|---|---|
| Лог on-path ветки | `topic_card_versions_purge`, **не** `_skipped` | `{"deleted": 0, "table_size": 1285, "keep_last_n": 50, "retention_days": 180, "cutoff": "2026-02-13T03:30:00Z", "duration_s": 0.306}`; строк `_skipped` — **ноль** |
| Гейдж `tg_topic_card_versions_rows` | уходит с `0.0` на размер таблицы | `1285.0` |
| Counter `tg_topic_card_versions_purged_total` | остаётся `0.0` | `0.0` |

Сверка: `SELECT count(*) FROM topic_card_versions` = **1285**, то есть гейдж не «какое-то число», а именно текущий размер. Контейнер за это время не пересоздавался (`StartedAt` по-прежнему `2026-08-11T23:12:51Z`), значит тик отработал тот же процесс, которому knob выставили.

**Что именно доказано, помимо «сработало».** Строка `retention_days: 180` пришла **из самого scheduler-синглтона**, а не из свежего процесса — это то поведенческое доказательство, которого принципиально не мог дать `docker exec … python -c` (false-green из BUG-078 и BUG-092). Плюс `duration_s: 0.306` показывает, что window-CTE скан на текущем объёме дёшев, то есть выбор ночного часа был перестраховкой, а не необходимостью.

Первое реальное удаление ожидается ближе к концу октября 2026, когда genesis-версиям от 2026-04-26 исполнится 180 дней. До тех пор механизм проверен, но ничего не удаляет — ровно тот холостой прогон, ради которого включение делали при `WOULD purge: 0`.

### Update 2026-08-11 — Событие B выполнено; статус НЕ переводился до первого тика
...
Статус намеренно остался `Proposed` на сутки: условие приёмки ниже говорит «после
первого **prod-запуска**», а включение — это ещё не запуск. До тика утверждение
«работает в проде» было бы истинным при полном бездействии механизма, а такие
формулировки приёмке не подлежат. Тик состоялся `2026-08-12 03:30 UTC` — см.
`Update 2026-08-12` выше; статус переведён.
