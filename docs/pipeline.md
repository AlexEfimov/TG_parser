...
Начиная с версии 3.0 доступен альтернативный подход к обработке — **Multi-Agent Architecture**.

### Отличие от классического Pipeline

| Аспект | Pipeline v1.2 | Multi-Agent v3.0 |
|--------|---------------|------------------|
| Архитектура | Монолитный pipeline | Распределённые специализированные агенты |
| Управление | Последовательные этапы | Оркестратор + динамическая маршрутизация |
| Расширяемость | Изменение кода pipeline | Добавление новых агентов в registry |
| Мониторинг | По этапам pipeline | По агентам с детальной статистикой |
...
┌──────────────────────────────────────────────────────────────────┐
│                     OrchestratorAgent                            │
│   Координация workflow, маршрутизация задач между агентами      │
└──────────────────────────────────────────────────────────────────┘
         │                    │                    │
         ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│ ProcessingAgent │  │TopicizationAgent│  │   ExportAgent   │
│ (text, topics,  │  │ (clustering,    │  │ (ndjson, json)  │
│  entities)      │  │  topic cards)   │  │                 │
└─────────────────┘  └─────────────────┘  └─────────────────┘
```
...
- **Pipeline v1.2** — стабильный production режим с проверенным качеством
- **Multi-Agent v3.0** — для сложных документов, кастомных workflow, расширяемых систем

Подробная документация: [Architecture](architecture.md#multi-agent-architecture-v30), [USER_GUIDE](USER_GUIDE.md#multi-agent-architecture-v30--new).
