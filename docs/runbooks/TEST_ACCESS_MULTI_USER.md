...
**BUG-094 (открыт) — не меняйте настройки существующего канала через `add_channel`.** Повторный `add_channel` на канал, у которого уже есть история, сбрасывает `last_post_id` и `backfill_completed_at` в `NULL` (частичный `Source` против full-row upsert), и следующий инкрементальный тик заново вычитывает весь канал из Telegram. Данные не теряются — raw-записи идемпотентны, обработка пропускает уже обработанное, — но это лишний трафик и риск rate-limit. Касается всех, включая вас как админа. До фикса меняйте `include_comments` / `batch_size` прямо в БД: `UPDATE sources SET include_comments = … WHERE source_id = …`. Тестировщикам стоит сказать то же самое: «добавил канал — больше `add_channel` на него не вызывай».
