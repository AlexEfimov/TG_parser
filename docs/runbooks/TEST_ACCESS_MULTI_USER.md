...
|---|---|---|---|
| Кому | тем, кто тестирует продукт целиком: подключение, добавление канала, поиск, Q&A | тем, кому нужен результат, а не интерфейс | тем, кто должен искать по **вашей** базе знаний |
...
То же ограничение бьёт **между тестировщиками**. Канал в инстансе глобально уникален: кто первый сделал `add_channel("@foo")`, тот владелец; второй получает `No access to channel foo` и канала в своей базе не видит. Telegram при этом остаётся публичным. Живой кейс 2026-08-31 (`physics_of_business`, Alamogordo777 / Ye_Ale) и открытые варианты модели — [`OPEN_CHANNEL_GLOBAL_OWNERSHIP_2026-08-31.md`](../notes/OPEN_CHANNEL_GLOBAL_OWNERSHIP_2026-08-31.md). Пока решения нет: разводите когорту по разным каналам и просите `@username`, не `t.me/…` (URL не нормализуется и плодит отдельные строки).

Живой замер (свежий `user`-токен против прода с уже наполненной базой админа): `list_channels` → `[]`, `list_topics` → `total=0`, `search_knowledge_base(...)` → `[]`, `pause_channel("<канал админа>")` → `No access to channel …`, `list_users` → `Admin access required`. Пусто — это корректная работа изоляции, а не поломка.
...
Два подвоха ручного пути:

- **не опускайте `max_channels`.** При `NULL` действует глобальный `DEFAULT_MAX_CHANNELS`, а он равен **20** — тестировщик молча получает право завести двадцать каналов на ваших LLM-ключах. Скрипт поэтому всегда передаёт явное значение (по умолчанию 3);
- **`mapping_id` больше нигде не отдаётся** — ни один MCP-tool не перечисляет auth-mapping'и. Не записали — достаётся только SQL'ем (§7).
...
**BUG-093 — закрыт и задеплоен 2026-08-12.** До фикса `add_channel` с id **существующего** канала попадал в `upsert_source` в обход проверки владения: чужой токен не получал доступ на чтение (`owner_id` сохранялся), но перезаписывал всю строку источника — вплоть до сброса курсора ingestion, — а в боте ещё и показывал текущий статус чужого канала в preview. Проверено на проде после деплоя: `add_channel` на канал админа под `user`-токеном → `No access to channel …`, строка источника не изменилась. Если вы деплоите свой инстанс с образа старее 2026-08-12 — фикс в нём отсутствует.

**BUG-094 — закрыт и задеплоен 2026-08-14; workaround снят 2026-08-15.** Повторный `add_channel` на канал с историей больше не затирает `last_post_id` / служебные поля: overlay кладёт только переданные аргументы. Живой owner-smoke на проде (`medportal_rfed`, только `channel_id`) → `created=false`, курсор `123` жив. Если деплоите свой инстанс с образа старее 2026-08-14 — фикса в нём нет, тогда по-прежнему правьте `include_comments` / `batch_size` прямым `UPDATE sources`. CLI `add-source` без флагов по-прежнему утверждает `false`/`100` — это не курсор.
...
- **отзыв не мгновенный: до ~60 секунд токен продолжает работать полностью.** Кеш резолвера (`tg_parser/auth/resolvers.py::_CACHE_TTL = 60`) держит уже разрешённую пару «токен → пользователь», и `remove_user_auth` его не инвалидирует. Замер 2026-08-12 на живом проде: сразу после отзыва `initialize` отвечал `200` на t+1/11/21/32 с и `401` на t+42 с (отсчёт от последнего запроса тестировщика, а не от отзыва). Если отзыв срочный — после команды подождите минуту и перепроверьте; если очень срочный, кеш живёт в процессе, так что его сбрасывает только рестарт `tg_parser_mcp`;
- после истечения кеша MCP отдаёт **401** `invalid_token` — ровно как на запрос вообще без токена (проверено);
