...
> **Заведено 2026-08-12 под другими номерами.** Пока шла эта сессия, параллельные PR #392 и #396 заняли 093 и 094, поэтому находки уехали на **BUG-095 … BUG-098** в том же порядке, что в таблице ниже (доставка watchlist → 095, экспорт → 096, дедупликация → 097, покрытие → 098). Предложенные ниже номера оставлены как есть — они часть записи о том, что сессия видела в тот момент. Сам факт коллизии стал входом для сессии #2: у `BUG_LOG` нет единой точки выдачи id.
