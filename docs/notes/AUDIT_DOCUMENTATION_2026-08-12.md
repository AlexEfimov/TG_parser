...
3. **Справочник MCP отстал от сервера на три инструмента.** `rg -c '@mcp\.tool' tg_parser/mcp_server.py` → **47**; шапка `MCP_AGENT_GUIDE` заявляла 43, её собственные таблицы перечисляют **44**. Не описаны ровно три F5-C-инструмента, два из которых #1 прогнала успешно. `README` вчетверо повторяет «43 tools» и «32 tools» при 47 и 35.
...
**Размер артефакта.** Лимит plan §7 — «≤3 стр. narrative вне таблиц»; страница в словах нигде не определена, поэтому вот измерение вместо оценки: `awk 'BEGIN{inb=0} /^```/{inb=!inb;next} inb{next} /^\|/{next} /^#/{next} {print}' <файл> | wc -w` → **1433** здесь против **917** у эталона #1. Разница — восемь обязательных секций шаблона §7 против четырёх у #1 и четыре потока работы против одного, а не запас прозы: таблицы, вывод замера и заголовки в счёт не идут.
