...
**Когда:** 2026-08-12, 15:12–16:0x UTC · **main@:** `a7171ff` · **Эталон:** [`AUDIT_FUNCTIONAL_EXECUTABLE_2026-08-12.md`](AUDIT_FUNCTIONAL_EXECUTABLE_2026-08-12.md)
...
Сверка со снимком `dbadc69` из plan §2: корпус не разъехался. Отличия — `docs lines` 137 479 → 137 614, `root lines` 7 436 → 7 463, `BUG_LOG` 5 803 → 5 905 (за это время в него легли BUG-095…098 из PR [#398](https://github.com/AlexEfimov/TG_parser/pull/398)). Остальные счётчики совпали до единицы.

**Свип битых ссылок** (команда plan §4), прогон на `a7171ff` до правок этой сессии: **207** попаданий — **140** в `archive/**`, **67** в живых документах, **5** root-anchored. Совпало со снимком плана точно.
...
**Вердикт по канону: частично.** Там, где канон назначен явно — баги, деплой, перевёрнутый обратно reverse-proxy-инвариант — он работает и указатели ведут куда надо. Там, где его не назначали, роль SoT занял тот документ, который чаще редактировали: статус-карту продукта де-факто держит 3 311-строчный бэклог, а «архитектуру системы» — документ с баннером «Historical / MVP», отсылающий к файлу вчетверо короче и про другое.

1. **F11 обещают пять живых документов, доставки нет с 2026-06-15.** `FUTURE_FEATURES:1818` («реализован полным контуром… push-уведомления»), `ROADMAP_KARPATHY:26` («instant push via aiogram») и `:302`, `MCP_AGENT_GUIDE:81` («chat_id receives instant pushes after each scheduler tick»), `README:13`, `USER_GUIDE:762`. Ни один не оговаривается. Это самый широкий из найденных разрывов: одно сломанное плечо продублировано в пяти местах.
2. **Стоимости нет ни в одном живом документе — и это не «наврали», а «нечем врать».** Число «$4–5/неделя», против которого #1 выставила измеренные $33.23 / ≈$16, живёт только в датированных `mcp_testing/2026-05-15…` (5 файлов, измерение другой конфигурации) и в SUPERSEDED prep в `archive/`. В живом корпусе `$/неделя` и `$/документ` отсутствуют как класс; единственная стоимостная величина в живых документах — `$215–380` восстановления из ADR-0021, и она с прогоном согласна. Риск не в ложном числе, а в том, что сессия #4 пойдёт за единственным сохранившимся — архивным.
3. **Справочник MCP отстал от сервера на три инструмента и на четыре в счётчике.** `rg -c '@mcp\.tool' tg_parser/mcp_server.py` → **47**; в шапке `MCP_AGENT_GUIDE` стояло «Tools: 43», в её собственных таблицах — **44**. Не описаны ровно три F5-C-инструмента: `get_topic_versions`, `get_topic_history_diff`, `force_resummarize`. Два первых #1 прогнала успешно. `README` в четырёх местах повторяет «43 tools» и «32 tools» при 47 и 35 фактических.
4. **Расхождение в обратную сторону: F1 работает и нигде не отмечен как сделанный.** `prompts/` содержит 10 YAML + README, `PROMPTS_DIR` — настройка, `reload_prompts` — живой MCP-инструмент из тех же 47. В сводной таблице `FUTURE_FEATURES` у F1 нет отметки DONE, в теле секции `## F1` — тоже. Там же F2 помечен `✅ DONE` в шапке документа (стр. 7) и в теле (стр. 311), но **не** в сводной таблице (стр. 21) — карта статусов расходится сама с собой на 290 строк.
5. **Дедупликация после LLM была описана за месяц до того, как её «нашли».** `CODE_REVIEW_PROCESSING_ALGORITHMS_FABLE5_2026-07-07:199` (F-01, severity High): «Точный дедуп срабатывает после LLM… каждый точный репост = полный оплаченный processing-вызов». `USER_GUIDE:467–488` при этом описывает только экономию («embedding не генерируется») и молчит про уже оплаченный вызов. Находка существовала в корпусе, не была связана ни с пользовательской документацией, ни с багом — до BUG-097 от 2026-08-12. Дефект здесь не в тексте, а в том, что у ревью-артефакта нет пути в канон.
...
**Про `stale-status` = 0.** Класс сработал, но не на возможностях, а на метаданных документов: «Tools: 43» при 47, «43 tools» / «32 tools» в `README` при 47 / 35, `Last Updated: April 25, 2026` в `PRODUCTION_DEPLOYMENT` при контенте от июня и августа, `Updated: June 2026` в `SERVER_ARCHITECTURE` при секции от 2026-08-11, `Обновлён 2026-03-30` в `technical-debt-roadmap` при закрытиях от 2026-08-10…11. Разносить это по строкам матрицы было бы подгонкой: счётчик инструментов относится сразу к тридцати строкам. Записано в §3 и исправлено в §5.
...
**Сначала — про метрику «94».** Она непригодна как счётчик притязаний, и починить её регуляркой не вышло. Декомпозиция: `rg -il 'SoT scope:'` → **4** (форма-указатель); `rg -il 'этот файл — SoT|— SoT по|is the source of truth|единый источник правды|единственный источник правды'` → **14**. Но и эти 14 не притязания: восемь — отработавшие `START_PROMPT_*`, где заявление ограничено своим спринтом, а в `FUTURE_FEATURES:2419` и `ROADMAP_V3:454` «единственный источник правды» сказано про **alembic и схему БД**, а не про сам файл. То есть регулярное выражение не отличает «я канон» от «канон — вон там» и от «канон вообще не документ». **Метрика записана как непригодная**; таблица ниже собрана чтением кандидатов, а не поиском.
...
67 живых попаданий (свип считает уникальные пары «источник → цель») разложены на «неверная база», то есть цель существует и путь считан не от той точки — **56**, и «цель отсутствует» — **11**. Второе опаснее, и оно почти всё в отработавших промптах:
...
Как искать открытые баги, не читая 5 905 строк — **уже решено, и решено правильно**, в тот же день 2026-08-12. Блок «⚠️ Что открыто прямо сейчас» несёт шесть строк, знаменатель приходит со своей командой (`rg -o 'BUG-[0-9]{3}' docs/notes/BUG_LOG.md | sort -u | wc -l` → 98), а workflow объявляет этот блок единственным местом, которое обновляется при смене статуса. Проверено: `sed -n '/## ⚠️ Что открыто/,/^Этот блок/p' … | rg -c '^\| \[?\*\*BUG-'` → 6. Новой политики роста для тела файла не нужно — тело растёт, а вход в него один и дешёвый.

**Что осталось предложить — только запрет на второй вход:** запись в журнал без парной строки в блоке считается незавершённой. Иначе шапка деградирует ровно тем же способом, каким деградировал сам файл.

**Выдача id — вот здесь дефект.** Сессия #1 предложила 093–096; пока она шла, PR [#392](https://github.com/AlexEfimov/TG_parser/pull/392) и [#396](https://github.com/AlexEfimov/TG_parser/pull/396) заняли 093 и 094, и находки уехали на 095…098 (заведены PR [#398](https://github.com/AlexEfimov/TG_parser/pull/398)). Причина — не невнимательность, а рукописный счётчик без единой точки выдачи.
...
> ```
> На `a7171ff` выдаёт `BUG-099`. Любое зафиксированное заранее «свободно 095» протухает за часы и резервированием не является. Планирующие документы называют находки описательно («доставка watchlist»), номер выдаётся при коммите в `BUG_LOG`.
>
> **Префикс `10#` — не украшение.** Первая редакция этого правила была напечатана без него и падала на `098`: bash читает ведущий ноль как восьмеричную запись, а `8` — недопустимая восьмеричная цифра. Команда выглядела рабочей ровно до первого запуска — тот самый дефект, ради поиска которого написан plan §2, воспроизведённый внутри артефакта, который его ищет.

### `FUTURE_FEATURES.md` (3 311 строк)

**Разделять.** Документ сам заявляет две роли — «Living backlog + shipped-status map» — и они уже разошлись: F2 отмечен DONE в шапке (стр. 7) и в теле (стр. 311), но не в сводной таблице (стр. 21); F1 не отмечен нигде при живом `prompts/` (10 YAML) и `reload_prompts` среди 47 инструментов. Статус-карта обязана быть короткой и сверяемой с прогоном; бэклог обязан быть длинным и сверяемым не с чем. Пока они в одном файле, читатель не знает, какую из двух дисциплин к строке применять.
...
Разделять не нужно: жанр один. Нужно другое — **у него нет владельца поверхности**. Три из его расхождений (дедупликация без цены, отсутствие HTTP-кодов, `../../`-ссылки) держались потому, что документ никем не сверяется с прогоном.

> **Правило:** каждая функциональная секция `USER_GUIDE` несёт строку «сверено с прогоном: `<дата>`». Несверённая секция — кандидат в подозреваемые, а не в источники.

### Правило для 107 `START_PROMPT*` на верхнем уровне

107 отработавших промптов против 53 файлов в `archive/` — примерно четверть `docs/notes/` лежит вперемешку с живыми документами и тянет за собой 53 из 56 битых ссылок класса «неверная база».

> **Правило.** `START_PROMPT_*` (и парный `PLAN_*`) переезжают в `docs/notes/archive/` **в том PR, который закрывает их сессию** — это последний момент, когда автор ещё знает, закончилась ли работа. Промпт на верхнем уровне означает «сессия не закрыта»; иного значения у расположения нет. Разбирать накопившиеся 107 поштучно не нужно и вредно: единственная безопасная миграция — по факту смерженного PR, поэтому накопленное уезжает одной исполнительной сессией со списком, собранным командой, а не руками.
...
Свип после правок (та же команда plan §4): **205** попаданий против 207, живой слой **65** против 67, `archive/` без изменений — **140**. Минус две уникальные записи `USER_GUIDE`; ссылка на этот артефакт из фикса № 4 закрылась его созданием.

Три слота из десяти не израсходованы намеренно: остальные кандидаты требуют решения (куда целить баннер `architecture.md`), а не указателя.
...
2. **Оговорка к `download_url`** в `MCP_AGENT_GUIDE:375–377` и `:948` — до фикса BUG-096.
3. **`ENV_VARIABLES_GUIDE.md`** — врезка про compose allow-list со ссылкой на `SERVER_ARCHITECTURE.md` § Configuration channel. Единственный незакрытый пункт W1-b.
4. **Счётчики инструментов в `README.md`** (4 места) — по образцу фикса № 4; §6 плана правку по коду не разрешала.
5. **Три секции F5-C в `MCP_AGENT_GUIDE`** — `get_topic_versions`, `get_topic_history_diff`, `force_resummarize`.
6. **Разделение `FUTURE_FEATURES`** на статус-карту и бэклог; попутно F1 → DONE и F2 в сводной таблице.
7. **Переезд отработавших `START_PROMPT*`** одной миграцией по правилу §4.
8. **Битые ссылки `BUG_LOG` ×2** — перецелить не на что; решение владельца: положить артефакты сессии 2026-05-16 в репозиторий или снять ссылки.
...
**Вход для #3 (код-ревью):** `mcp_server.py:314` — описание сервера обещает `raises`, шесть его же docstring'ов обещают пустой результат, и прогон согласен с docstring'ами. **Вход для #4:** стоимости в живом корпусе нет; брать из #1, не из `archive/PLANNING_AUDIT_AND_STRATEGY_PREP:88`. **Вход для #5:** роадмап-жанр сведён (7 по имени + 7 по содержанию), вакансии канона названы в §2.

---

## 7. Не сделано и почему

Бюджет не резался: W1 и W1-b пройдены полностью, W2 и W3 — тоже. Сознательно не сделано следующее.

- **Живой слой ссылок разобран классами, а не поштучно.** 56 из 67 сидят в отработавших промптах; поштучный разбор противоречил бы правилу §4 «нужно правило, а не разбор» и был бы выброшен ближайшим переездом.
- **`docs/notes/mcp_testing/**` и `docs/notes/archive/**` только грепались**, как требует plan §4 и Hard OUT. 140 битых ссылок в archive сосчитаны и не тронуты.
- **Отработавшие `START_PROMPT*` и `docs/prompts/**` не читались** — только счёт и правило.
- **Ни одна диспозиция `archive` / `merge-into` / `delete` не исполнена** — по §5 и §6 они предложения.
- **Метрика «94 файла упоминают SoT» признана непригодной**, а не интерпретирована: команда, отделяющая притязание от ссылки, не нашлась, и её отсутствие записано вместо вывода. Таблица §2 собрана чтением.
