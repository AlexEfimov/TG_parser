...
> - **Observability:** `write_intent_set` / `_router_resume` / `_router_failed` / `_declined` / `_dropped` (+ `reason`), плюс `fsm_confirm_declined` (закрывает отдельный GAP §3).
...
> 2. **Дельта тестов положительная (+44)**, а не «−17 / +12-15» — см. выше.
>
