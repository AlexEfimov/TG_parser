...
# Re-snapshot t2 (2026-07-16, попытка #2 — scheduled cron)

**Тип:** запланированный (cron `0 13 * * *`) read-only re-snapshot трёх Phase-1 окон против t0-baseline. Единственный записанный артефакт — эта секция.
**Итог сбора:** **прод недоступен для сбора метрик из этого Cloud-Agent VM** — нет SSH-креденшелов. Ни одно значение t2 не снято и **ничего не сфабриковано** (см. §t2.0). Все три watch помечены **INCONCLUSIVE (t2 не собран)**; их последний фактический статус — из t1 (2026-07-16T10:52Z).

## t2.0 Контекст снятия и gap доступа

```text
T2_UTC (попытка):           2026-07-16T13:03Z   (cron triggeredAt 2026-07-16T13:02:17Z)
ELAPSED since t0:           ≈ 24h41m   (t0 = 2026-07-15T12:22:05Z)
ELAPSED since t1:           ≈ 2h11m    (t1 = 2026-07-16T10:52:36Z)
СБОР МЕТРИК:                 НЕ ВЫПОЛНЕН — прод недостижим (нет SSH-ключа в этом VM).
```

> ⚠️ **Gap #5 — прод недостижим из runner'а (нет креденшелов).** Плановый сбор t2 не состоялся. Диагностика попытки доступа (строго read-only, никаких изменений):
> - **TCP:** `212.72.189.15:2296` — открыт/достижим (egress не ограничен: `restricted:false`).
> - **SSH-хост `prod`:** алиас не сконфигурирован в этом VM (`Could not resolve hostname prod`); `~/.ssh/` отсутствует целиком.
> - **Auth:** сервер предлагает `publickey,password`; в VM **нет** приватных ключей (`~/.ssh/id_*` отсутствуют), нет `ssh-agent`, нет секрет-env (`SSH_*`/`PROD_*`/пароля). Прямое подключение к `user@212.72.189.15:2296` → **`Permission denied (publickey,password)`**.
> - **Причина:** этот Cloud-Agent run стартовал just-in-time без сохранённого environment (`environment:null`); SSH-секрет, которым пользовались прошлые снапшоты (t0/t1), в этот VM не проброшен.
> - **Это не прод-инцидент:** недоступность — на стороне runner'а (отсутствие ключа), а не прода. Здоровье прод-сервисов по этому чтению **неизвестно** (не проверялось).

## t2.1 Verdicts (t2)

| Watch | Target | Elapsed на t2 | Последний факт. статус (t1) | Verdict t2 |
|---|---|---|---|---|
| **W1** BUG-084 embedding quota / alert soak | 24–48h | ≈24h41m | PASS (0 rate_limited/quota_exhausted; алерты inactive) | **INCONCLUSIVE (t2 не собран)** — прод недостижим |
| **W2** S3 pre-LLM dedup (billing-clean 48–72h read) | 48–72h | ≈24h41m | INTERIM (too-early; pre-LLM hits=0; 24h billing-clean) | **INCONCLUSIVE (t2 не собран)** — прод недостижим; **и всё равно too-early** (см. ниже) |
| **W3** S5/S6 post-deploy | 24–48h | ≈24h41m | PASS (malformed_merge=0; coverage ≥ baseline) | **INCONCLUSIVE (t2 не собран)** — прод недостижим |

**Аномалии (t2):** не наблюдались и не могли наблюдаться — метрики не читались. Отдельно фиксируется единственная аномалия — **инфраструктурный gap доступа** (Gap #5).

## t2.2 W2 / S3 — фокус: FINAL или INTERIM?

Задача просит FINAL-вердикт по W2, **если** elapsed попал в/за окно 48–72h и окно было billing-clean с t0. Проверка по времени (независимо от gap'а доступа):

- **Elapsed на t2 ≈ 24h41m** — это **НИЖЕ** нижней границы целевого окна W2 (**48h**). Даже при успешном чтении сегодня вердикт был бы **INTERIM (too-early)**, а не FINAL — первый корректный billing-clean long-window read возможен не раньше **2026-07-17T12:22Z (48h)**.
- Следовательно: **W2 НЕ достиг FINAL** ни по данным (t2 не собран), ни по времени (24h41m < 48h).
- **Автоматизацию отключать НЕЛЬЗЯ.** Её цель (финальный billing-clean 48–72h read по W2) ещё не выполнена.

## t2.3 Рекомендуемое следующее действие

1. **Починить доступ (блокер):** пробросить SSH-секрет прода в Cloud-Agent (Cursor Dashboard → Cloud Agents → Secrets: приватный ключ + при необходимости `~/.ssh/config` алиас `prod`), либо привязать сохранённый environment, которым пользовались t0/t1. Без этого никакие будущие тики не соберут метрики.
2. **Повторить W2-read в billing-clean окне 48–72h:** ближайший валидный запуск — **2026-07-17T12:22Z → 2026-07-18T12:22Z**. Там же — подтверждающий read W1/W3 на 48h (при отсутствии изменений закрыть оба как PASS).
3. **До получения FINAL по W2 — оставить автоматизацию включённой.**

*Попытка read-only сбора 2026-07-16T13:03Z из Cloud-Agent VM (`bc-6f009cc6...`): прод SSH недостижим из-за отсутствия креденшелов в runner'е. Метрики t2 НЕ сняты; ни одно значение не сфабриковано — вместо чисел зафиксирован явный gap доступа (Gap #5).*
