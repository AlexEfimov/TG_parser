...
**Next (open, unblocked):** decision-input → [`DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md`](DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md). Owner sequence: **ε+ζ** session ([`START_PROMPT_SESSION_EPS_ZETA_…`](START_PROMPT_SESSION_EPS_ZETA_INTERNAL_FILL_TTL_SKELETON_2026-07-20.md); ζ skeleton → [`SKELETON_F5C_TTL_RETENTION_…`](SKELETON_F5C_TTL_RETENTION_TOPIC_CARD_VERSIONS_2026-07-20.md)) → **δ/T7** после +24ч/+48ч watch. Passive: T7 watch (hold `=14` until verdict); γ1′ checklist → [`BUG008_RECURRENCE_CHECKLIST.md`](BUG008_RECURRENCE_CHECKLIST.md). Parking-lot (Wave E, F11 HTTP CRUD, webhook 2A, S4 multilang) — gated на 2A/2B/2C > 0. «Wave 3» entry — только после явного выбора контракта.
