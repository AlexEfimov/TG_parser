...
**Next (open):** **δ/T7 CLOSED** — bump `14 → 21` **APPLIED** 2026-07-22T19:49Z (prod `RESUMMARIZE_MAX_AGE_DAYS=21`, OS-env verified, health `healthy`; [`DELTA_T7_VERDICT_2026-07-22.md`](DELTA_T7_VERDICT_2026-07-22.md)). 🔔 **OPEN follow-up — re-watch checkpoint ≈ 2026-08-05:** снять `ratio14d`/alert после полной прокрутки trailing-14d окна (post-apply `≈0.987` = ожидаемый lag). `<0.5`+alert cleared → 21 подтверждён (закрыть); иначе рассмотреть `21 → 30` или accept steady-state. Детали/команды: [`DELTA_T7_VERDICT_2026-07-22.md`](DELTA_T7_VERDICT_2026-07-22.md) § Open follow-up, runbook §T7. ε+ζ ✅ (#342/`f4073a6`). **TTL/retention для `topic_card_versions` (#15 item #1) — ✅ CODE DONE 2026-07-22 (prod-gated):** hybrid keep-last-N ∧ older-than-M ∧ genesis-pin, hard-DELETE zero-schema, daily cron + CLI `topic purge-versions --dry-run` + gauge/counter; default `RESUMMARIZE_VERSION_RETENTION_DAYS=0` (kill-switch), prod `180`/`KEEP_LAST_N=50` по in-session owner GO + backup + dry-run ([ADR-0018](../adr/0018-topic-card-versions-retention.md), [plan](PLAN_SESSION_F5C_TTL_RETENTION_2026-07-22.md)). **Deploy-split:** «Событие A» (выкатить код default-off, no-op) — на ближайшем штатном окне; **«Событие B» (флип `RETENTION_DAYS=180`) — рассмотреть на том же re-watch δ/T7 checkpoint ≈ 2026-08-05** (см. runbook § «Событие A / Событие B» + [`DELTA_T7_VERDICT_2026-07-22.md`](DELTA_T7_VERDICT_2026-07-22.md) § Open follow-up). Baseline 2026-07-23: would_purge=0. **diff-API (#15 item #2) — ✅ CODE DONE 2026-07-23:** read-only `get_topic_history_diff` (MCP) + `tg-parser topic diff` (CLI) поверх общего pure-helper `diff_topic_summaries` (`difflib` summary text-diff + `scope_in`/`scope_out` set-diff); dual-source (archival `version_no` pairs `topic_card_versions` **+** `current`/`latest` → live card `topic_cards`), default genesis (v1) → current; robust к post-TTL `version_no` gaps (ADR-0018) — typed not-found «reclaimed by retention policy», не 500; repo read-path `TopicCardVersionRepo.get_two_versions`; no schema-change, no new deps ([plan](PLAN_SESSION_F5C_DIFF_API_2026-07-23.md), [START_PROMPT](START_PROMPT_SESSION_F5C_DIFF_API_2026-07-23.md)). Parking-lot: Wave E, F11 HTTP CRUD, F6 topic-digest #3 / Bot-tools #5 — gated на 2A/2B/2C > 0 or explicit GO. Decision-input: [`DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md`](DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md).
