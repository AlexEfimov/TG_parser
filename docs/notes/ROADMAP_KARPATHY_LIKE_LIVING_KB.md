...
**Next (open):** **δ/T7 CLOSED** — bump `14 → 21` **APPLIED** 2026-07-22T19:49Z (prod `RESUMMARIZE_MAX_AGE_DAYS=21`, OS-env verified, health `healthy`; [`DELTA_T7_VERDICT_2026-07-22.md`](DELTA_T7_VERDICT_2026-07-22.md)). Passive re-watch: `ratio14d`/alert ожидаемо red до прокрутки trailing-14d окна (post-apply `≈0.987`); age-share должен снижаться. ε+ζ ✅ (#342/`f4073a6`). Parking-lot: TTL impl (ζ skeleton landed), Wave E, F11 HTTP CRUD — gated на 2A/2B/2C > 0 or explicit GO. Decision-input: [`DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md`](DRAFT_NEXT_CONTRACT_POST_GAMMA_CLOSEOUT_2026-07-20.md).
