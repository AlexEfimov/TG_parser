...
| T+12h00 | workspace CRUD + search/ask | [x] | `2026-05-23T09:15Z … 09:17Z` (13:15–13:17 MSK 23-05) · target=**MCP `user-tg-parser` (prod, `mcp.tgp.efimov.mobi`)**. **A1** `create_workspace(name=workspace_watch_smoke, description="24h watch — DELETE on cleanup")` → `workspace_id=029895fc-6b29-4946-b773-1f06fd8045e1`, `owner=c59d42b4`, created. **A2** `add_workspace_source(workspace_id=029895fc, channel_id=mind_rise)` → `changed=true`; затем `add_workspace_source(channel_id=genotek)` → `changed=true`. **A3** `list_workspace_sources(029895fc)` → `count=2`, `channel_ids=[genotek, mind_rise]` ✓. **A4** `search_knowledge_base(query="health longevity", limit=5, mode=hybrid, workspace_id=029895fc)` → 5 hits, top scores `0.0164` (`tg:genotek:post:1352` longevity-genetics-UK-Biobank; `tg:mind_rise:post:1460` sleep-deprivation), последующие `0.0161`/`0.0159` все в scope (`mind_rise`+`genotek`); workspace-scoping работает корректно (никаких других каналов в выдаче). **A5** `ask_question(question="Назови один биомаркер старения", mode=hybrid, workspace_id=029895fc)` → **FAILED**: `Your credit balance is too low to access the Anthropic API` (RAG default-config использует Claude); записано как anomaly, не блокер для CRUD/search-валидации. **A6** `remove_workspace_source(workspace_id=029895fc, channel_id=genotek)` → `changed=true`; verify `list_workspace_sources` → `count=1`, `channel_ids=[mind_rise]` ✓. **Anomaly:** ask_question отвалился по quota — не баг pipeline, а exhaustion внешнего LLM-провайдера; closure-session может зафиксировать в Open Items (RAG provider replenish / fallback policy). |
...
| T+14h00 | export_channel raw mind_rise | [x] | `2026-05-23T09:17Z` (13:17 MSK 23-05) · target=**MCP `user-tg-parser` (prod, `mcp.tgp.efimov.mobi`)**. **B1** `export_channel(channel_id=mind_rise, level=raw, format=ndjson, from_date=2026-05-23T00:00:00Z, to_date=2026-05-23T09:15:00Z)` → `job_id=9a5cfb21-c18c-4d05-8ddc-0ecfdc2829e6`, `status=pending`. **B2** первый `get_export_status` (через ~3s после dispatch) → `status=completed`, `download_url=/api/v1/export/download/9a5cfb21-…`, `file_size=0`. Размер 0 байт ожидаем: окно `00:00–09:15Z` сегодня в `mind_rise` пусто (last_success_at ingestion = `08:54Z`, новых постов в окне не было; raw export парс-онли возвращает то, что есть в Telegram за окно). Job завершился внутри ms — поллинг 3-шагового цикла не понадобился; ingestion baseline не задет. **Anomaly:** нет — пустой результат корректен для узкого окна. |
...
| T+15h00 | CLEANUP (§ 5) + final sweep — clean state | [x] | `2026-05-23T09:17Z … 09:25Z` (13:17–13:25 MSK 23-05) · target=**MCP `user-tg-parser` (prod) + SSH/Prometheus snapshot**. **C1** `unsubscribe_watchlist(interest_id=90d5e821-5702-456f-a00b-75703f85e244)` → `success=true`, «Interest deactivated» (`wl_watch_smoke` soft-deleted per F11 contract — row preserved, `is_active=false`). **C2** `unsubscribe_digest(subscription_id=ca96b614-5879-4107-8922-65cc0b108256)` → `success=true`, «Subscription deleted» (`digest_watch_smoke` removed; scheduler job отзаписан атомарно). **C3** `delete_workspace(workspace_id=029895fc-6b29-4946-b773-1f06fd8045e1)` → `success=true`, «Workspace deleted» (CASCADE M2M; underlying `sources` rows preserved per F4-B contract). **C4** `get_pipeline_status` для всех 3 каналов: `mind_rise` active `fail_count=0` `last_success=2026-05-23T08:54:11Z`; `genotek` active `fail_count=0` `last_success=2026-05-23T08:55:59Z`; `AgeManagment` active `fail_count=0` `last_success=2026-05-23T08:49:04Z` — все ингёршн-ранs от scheduler-tick завершились чисто, никаких stuck jobs от catch-up triggers. **C5** Sanity sweep: `list_watchlists count=12` (5 prod + 7 smoke remnants все `is_active=false`: `385b4b41` S3 smoke, `abfbfbf9` Idem, `1eac40cd` _smoke_post91_, `604632d4` wl_bot_watch_smoke, **`90d5e821` wl_watch_smoke ✓ ТОЛЬКО ЧТО**, `128901f2` http_smoke_T3, `f75326de` http_smoke_T15) — **0 active `_watch_smoke` artifacts**; `list_digests count=1` (только prod-эндокринология `94483db9`, **0 `_watch_smoke`**); `list_workspaces count=3` (только 3 prod: Лабдиагностика, Longevity, Эндокринология — **0 `_watch_smoke`**, `029895fc` ушёл из active set hard-delete). **wl_bot_watch_smoke** (`604632d4`) уже был `is_active=false` с 22-05 20:08Z (user-delete в bot session, документировано в § T+2h00…T+2h25). **Verdict:** state clean, готов к closure. |
| T+15h00 | Prometheus final snapshot (post-cleanup baseline for closure) | [x] | `2026-05-23T09:25:57Z` (13:25 MSK 23-05) · target=**`docker exec tg_parser_prometheus wget /api/v1/query`** (SSH loopback). **Counters (api scope) — Δ от T+14h46 baseline** (`08:30:57Z … 09:25:57Z`, окно ≈55 мин без новых HTTP/MCP write-ops): `tg_idempotency_keys_hit_total{result=miss} 4→4 (Δ=0)`; `{result=hit} 4→4 (Δ=0)`; `{result=mismatch} 3→3 (Δ=0)` — ожидаемо: в Part A/B/C MCP-вызовы не несут `Idempotency-Key`, поэтому middleware-счётчики статичны. `tg_idempotency_keys_table_size{api} 4→5 (+1)` (новый row, скорее всего residual idempotency entry от любого scheduler-tick'а с Idem-Key поверх API; hourly cleanup `0 * * * *` запустится в `10:00Z` — Δ=+1 не блокер); `{mcp}=0`; `{bot}=0`. **`tg_pipeline_trigger_total` (Δ от 08:30Z):** `full_pipeline{queued} 2→2 (Δ=0)`, `{success} 1→2 (+1)` — pipeline-run от T+14h46 catch-up успешно завершился; `topicization{queued} 1→1`, `{success} 0→1 (+1)` — topicization-job тоже finalised; `link_topics{queued} 2→2`, `{success} 2→2` — без изменений (был уже settled). **Все 3 job-метки имеют `result=success` ≥ 1, `surface=api`**; `surface=mcp`/`surface=bot` по-прежнему отсутствуют (архитектурный gap, уже документирован в § T+14h46 anomaly #2 и § T+2h00…T+2h25 / `surface=bot STILL EMPTY`). `up{service=api}=1` ✓ (без gap'ов; closure-session подтвердит full-range query). **READY FOR CLOSURE** — все `_watch_smoke` артефакты удалены / soft-deleted; closure session может proceed (`~14:25 MSK 23-05` / T+16h43). |
| T+15h45 | ask_question retry (Anthropic quota cleared — Open Item from T+12h) | [x] | `2026-05-23T09:30Z` (13:30 MSK 23-05) · target=**MCP `user-tg-parser` (prod, `mcp.tgp.efimov.mobi`)**. Retry T+12h failure (`Your credit balance is too low to access the Anthropic API` — баланс пополнен пользователем). Call: `ask_question(question="Назови один биомаркер старения", mode=hybrid, workspace_id=fb59c536-9ee8-4d6f-8208-17acacaa4e37)` (Longevity workspace) → HTTP/MCP **200 OK**, `model=claude-sonnet-4-20250514`, `answer_length=299` chars (RU, цитирует эпигенетические часы / DNA-метилирование), `sources count=5` (LongevityClub posts 106/184/247 + AgeManagment 542/30; top score `0.0164`). Log scan SSH+`docker logs --since 5m tg_parser | grep -iE "ask_question\|rag\|anthropic"` — пусто (только `/health` + `/metrics` scrape-traffic; никаких 5xx/exception). **Open Item cleared:** RAG provider replenish/fallback policy gap from T+12h closure prep — снят, фиксация Anthropic quota была transient external resource exhaustion, не дефект pipeline. Никаких новых артефактов: read-only RAG-вызов после T+15h cut-off допустим как closure-prep validation. |
| T+15h45 | hard cut-off, passive monitor only | [x] | `2026-05-23T09:30Z` post-retry состояние: ничего не создано/изменено в БД (RAG read-only); все `_watch_smoke` artifacts остаются soft-deleted/удалёнными per § T+15h00. Closure-session готов к запуску (`~14:25 MSK 23-05`). |
| T+22h09 | **closure session executed (1h17m early vs nominal T+24h)** | [x] | `2026-05-23T09:35Z` (12:35 MSK 23-05) · target=**Prometheus query_range/query на VPS + `docker logs` scan на всех 3 контейнерах + git operations на ноутбуке**. Closure-session ran ahead of nominal T+24h close (`~14:25 MSK 23-05`) by 1h17m; rationale: все артефакты cleaned by T+15h00, hard cut-off T+15h45 passed, no further write traffic planned (counters frozen ~5h). **Prometheus snapshot:** `up{service=api}` 88/89 = 1 (single zero at OPEN scrape only, 0 extended gaps); `tg_idempotency_keys_hit_total` `miss=4, hit=4, mismatch=3` (all three `result` labels populated ≥ 1); `tg_idempotency_keys_table_size{api}=5` (non-zero, gauge alive); `tg_pipeline_trigger_total` все 3 job-метки `success ≥ 1` под `surface=api`. **Log scan (4xx/5xx tally on `/api/v1/(watchlists\|digests\|pipeline)`):** `0 × 5xx`, `28 × 404`, `7 × 429`, `3 × 422`, `2 × 403` — все known classifications. **Container error tally:** `tg_parser` 942 events all `anthropic_billing_block_processing` (resolved external resource); `tg_parser_bot` 4 events (3× BUG-025 + 1× **NEW BUG-028** digest cron `PromptLoaderError` at `06:00:00Z` — pre-existing F6 latent bug surfaced for first time post-`d143e5d` restart; not a step 3 regression per git blame `410452a6` 2026-04-19); `tg_parser_mcp` 0 events. **Verdict: GREEN** per all 4 criteria of [`START_PROMPT_SESSION_WATCH_CLOSURE_2026-05-23.md` § 4](START_PROMPT_SESSION_WATCH_CLOSURE_2026-05-23.md). **Docs updated:** [`WATCH_WINDOW_WAVE1_STEP3_2026-05-22.md`](WATCH_WINDOW_WAVE1_STEP3_2026-05-22.md) (Status: CLOSED, full 24h query results, verdict GREEN); [`REVIEW_2026-05-21_WAVE1_STEP3_DONE.md`](REVIEW_2026-05-21_WAVE1_STEP3_DONE.md) (STATUS NOTE stub removed; §§ 2/3/6 filled; § 4a Open Items added; final appendix metrics); [`BUG_LOG.md`](BUG_LOG.md) (BUG-028 added). **Anomaly (BUG-028):** daily 09:00 MSK cron tick of prod endocrinology digest `digest_94483db9` fails with `missing prompt for required stage='digest': YAML at None/digest.yaml ...` — `str(settings.prompts_dir) == "None"` (Python literal) → path resolves to non-existent `None/digest.yaml`. **Workaround:** set `PROMPTS_DIR=/app/prompts` env on `tg_parser_bot` + restart (no rebuild needed) before next 09:00 MSK tick. Hotfix branch recommended: `fix/bug-028-digest-cron-prompt-loader`. **Next step:** Wave 1 step 4 planning sub-session (Shareable Digest, ADR 0008) — separate prompt to be created. |
