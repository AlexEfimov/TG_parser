...
> ✅ **LANDED & VERIFIED IN PROD (2026-07-26).** Фикс BUG-086 задеплоен ([PR #358](https://github.com/AlexEfimov/TG_parser/pull/358), prod `b6c21ef → 9aadf5e`), и **ручной e2e-smoke владельца в `@Tgingest_bot` — PASS** (07:50–07:56 UTC): dry-run терминален; мутационный запрос дал framework-preview; «да» → `ok`, версия `topic:tg:Docma_ru:post:252` **8 → 9**; негативный контроль на `topic:tg:mediamedics:post:2954` («нет» → «❌ Отменено.») оставил тему нетронутой на **9** версиях с прежним `last_summarized_at=2026-07-21T09:37:18Z`. Ошибок в окне — 0. **Structural guard слоя (c) НЕ срабатывал** (`llm_authored_confirm_*` = 0 за всё время жизни контейнера) — правильную форму вызова `confirm=false` дал prompt-слой (a); слой (c) остаётся непроверенным в проде belt-and-braces. **Побочная находка:** shadow-поле `read_only_intent` не залогировано ни разу — оно вычисляется **после** detector-гейта, т.е. пишется только когда guard сработал ⇒ на happy path выборка пуста **by construction** и FP-rate классификатора неизмерим (кандидат в [#359](https://github.com/AlexEfimov/TG_parser/issues/359)). Полные лог-выдержки: [`../runbooks/F5C_DEPLOY_AND_WATCH.md`](../runbooks/F5C_DEPLOY_AND_WATCH.md) § Deploy record — BUG-086 + `BUG_LOG.md` § BUG-086 строки «Production e2e verification» / «Shadow-mode observability GAP».
...
**Второй проход по точности (тот же review-цикл).** `подтвержда[еюя]\w*` матчился ещё и на **первое лицо** «Подтверждаю, что это только отчёт» и деепричастие «Подтверждая …» — это повествовательный текст, а не просьба; оставлены только второличные вопросительные формы (`подтверждаете` / `подтверждаешь`). Плюс добавлен **veto**: явный дисклеймер «подтверждение не требуется» (`_CONFIRMATION_DISCLAIMED_PATTERN`) отключает детектор целиком. Veto выбран вместо ослабления маркера `[да/нет]` сознательно — это прод-подпись и самый сильный признак просьбы, терять его нельзя. Зафиксирована и **управляющая асимметрия**, чтобы будущие проходы не расширили детектор обратно: пропущенная просьба стоит пользователю одной переформулировки, а ложное срабатывание молча подменяет запрошенный read-only отчёт мутационным preview и позволяет случайному «да» сжечь LLM-токены ⇒ **точность приоритетнее полноты**. Регрессии: `test_declarative_confirm_verbs_are_not_an_ask`, `test_explicit_no_confirmation_disclaimer_vetoes_the_detector` и `test_the_prod_trace_still_detected_after_every_precision_pass` (anti-over-correction — прод-трейс обязан детектиться после любого сужения).

**Третий слой точности — read-only-intent классификатор в SHADOW MODE (решение owner'а, 2026-07-25).** Логичное продолжение двух проходов выше — **вето**: не запускать recovery, если сам ПОЛЬЗОВАТЕЛЬ просил read-only отчёт («что будет, если …», «только покажи», «без запуска», «dry-run», «what would happen», «preview only»). Классификатор построен (`_READ_ONLY_REQUEST_PATTERN` + `_MUTATION_IMPERATIVE_PATTERN` → `_looks_like_read_only_request`), но **сознательно не подключён к поведению**: он добавляет одно булево поле `read_only_intent=` в уже существующие записи guard'а (`llm_authored_confirm_detected` / `llm_authored_confirm_recovered`) и **больше ничего** — control flow бит-в-бит прежний.

*Почему shadow, а не сразу enforce.* Этот класс FP наблюдался **только в ревью, ни разу в проде** — значит, список фраз, написанный сегодня, настроен против **воображаемых** формулировок, а не против реальных. Shadow mode превращает догадку в **измерение**: watch-окно показывает настоящую частоту прежде, чем кто-либо привяжет её к поведению. Критерий для флипа в enforcing — любая запись `llm_authored_confirm_recovered` с `read_only_intent=True` (guard вооружил мутационный preview на read-only запрос); пока таких нет, вето лечило бы несуществующую болезнь.

*Почему это безопасно включить потом — монотонность.* Вето умеет только **подавлять** арминг, но не создавать его. Значит, пропущенная классификатором формулировка = **сегодняшнее поведение** (recovery отрабатывает, как сейчас), а не новый дефект; риск флипа односторонний и ограничен ложным «read-only» на мутационном запросе — ровно поэтому «только якорные многословные фразы», голый глагол запрещён (иначе «пере-суммаризируй тему X и покажи, что получилось» стал бы read-only и enforcing-версия переоткрыла бы dead-end), а явный мутационный императив (`пере-суммаризиру(й|йте)`, морфологически отделённый от инфинитива `-ть` в легитимном «…если пере-суммаризировать X») сам является вето для read-only-вердикта.

*Privacy.* В лог идёт **только булево** — текст сообщения не логируется даже усечённым, по той же норме, что и соседний «только ключи аргументов» (`add_user_auth` несёт сырой credential в значении, а пользовательская реплика может процитировать его так же легко). Поле добавлено в watch-раздел [`../runbooks/F5C_DEPLOY_AND_WATCH.md`](../runbooks/F5C_DEPLOY_AND_WATCH.md), чтобы окно наблюдения реально проверялось.

**Четвёртый проход по точности — четвёртый FP-вектор, ИСПРАВЛЕН (2026-07-25, отдельный red→green слайс).** `подтвердит[еь]` смешивал императив «подтвердите» и **инфинитив** «подтвердить» — та же морфологическая ошибка, которую второй проход убрал у `подтвержда…`, но оставил на одну альтернативу правее. Легитимный read-only turn, пересказывающий payload'овский `next_step` («Это только отчёт. Чтобы **подтвердить** реальный запуск, попросите ещё раз»), детектился как самосочинённая просьба ⇒ отчёт подменялся мутационным preview, ConfirmFlow вооружался. Дисклеймер-veto не спасает: клаузу «no confirmation is pending» русский пересказ как раз и теряет. Сужение меняет control flow, поэтому shadow-слайс его принципиально не трогал. **Фикс:** `подтвердите\b|подтверди\b|подтвердить[^?.!\n]{0,40}\?` — императивы являются просьбой **по грамматической форме** (неформальное `подтверди` добавлено сознательно: FP-риска нет, закрывает небольшой пробел в полноте), а инфинитив засчитывается **только** во главе вопросительной клаузы («Подтвердить пере-суммаризацию?»), причём класс `[^?.!\n]` останавливает матч на границе предложения — декларативный инфинитив с несвязанным вопросом следом не матчится. Проверено через **композитный продовый хелпер** (scrub + veto + pattern), а не альтернативу в отдельности: **7** пересказов `next_step` перешли `True`→`False`, **6** просьб (3 императива + 3 вопросительных инфинитива) дают `True`, прод-трейс по-прежнему детектится. Регрессии: `test_infinitive_confirm_is_not_an_ask`, `test_imperative_and_interrogative_confirm_forms_are_still_asks` и end-to-end `test_dry_run_paraphrasing_the_confirm_infinitive_stays_terminal` (ровно ОДИН round-trip исполнителя, отчёт сохранён). Mutation-проверка: возврат `подтвердит[еь]` роняет все три, а end-to-end — с точным прод-симптомом (`preview_pending` вооружён, `read_only_intent=True` в `llm_authored_confirm_recovered`). Anti-over-correction-пин переименован в `test_the_prod_trace_still_detected_after_every_precision_pass` — без счётчика проходов, чтобы пережить следующее сужение.

**Пятый проход по точности — границы клауз; ПОСЛЕДНИЙ regex-проход (2026-07-25, отдельный red→green слайс).** Окно `подтвердить[^?.!\n]{0,40}\?` останавливалось только на границе **предложения**, но не на границе **клаузы**, поэтому декларативный инфинитив с коротким несвязанным вопросом следом снова склеивался в одну вопросительную клаузу: «Нужно подтвердить запуск — показать ещё раз?» и варианты с `,` / `:` / `;` детектились. **Фикс:** окно теперь обрывается и на разделителе клауз, но только там, где разделитель им реально является. Простой класс `[^?.!\n,;:—–-]` **отвергнут** — он ломает две из шести закреплённых просьб: внутрисловный дефис в «Подтвердить пере-суммаризацию?» и двоеточия в «Подтвердить запуск для темы topic:tg:c1:post:1?». Итоговая форма `подтвердить(?:(?![?.!\n])(?![,;:]\s)(?!\s[-–—])(?![-–—]\s).){0,40}\?` засчитывает `,;:` только перед пробелом, а тире — только окружённое пробелами (все три знака, которые реально встречаются в русском тексте: `-`, `–`, `—`). **Осознанный FALSE NEGATIVE, зафиксирован явно:** исключение клаузной запятой роняет и настоящую просьбу «Подтвердить, пожалуйста, запуск?». По управляющей асимметрии этот размен **верен** (пропуск = одна переформулировка; обратное = незапрошенный мутационный preview и сожжённые токены на случайное «да») и принят owner'ом, поэтому он закреплён отдельным тестом, у которого **имя и docstring** прямо говорят: это намеренное следствие правила «точность важнее полноты», а не недосмотр. Проверено через композитный хелпер: шесть новых текстов → `False`, 7 пересказов и 6 просьб четвёртого прохода без изменений, прод-трейс детектится. Mutation-проверка: возврат `[^?.!\n]{0,40}` роняет **ровно** два новых теста.

**Отложено (решение owner'а, 2026-07-25) — два оставшихся FP-вектора закрываются АРХИТЕКТУРНОЙ заменой, а не шестым regex-проходом.** Шесть FP-векторов за четыре раунда ревью, три из четырёх исправленных — морфологические: проблема в самом подходе, то есть в предсказании «просьбы» по свободной прозе LLM. Оставшиеся два регулярным выражением недостижимы: **(a)** `_CONFIRMATION_DISCLAIMED_PATTERN` чувствителен к порядку слов, а в русском он свободный — «подтверждение не требуется» вето срабатывает, а «Ничего подтверждать не нужно …», «Подтверждать этот отчёт не требуется …», «Подтверждения от вас не требуется …» — нет, и каждый пробел возвращает FP на маркере `[да/нет]` (именно вето позволяет этот — самый сильный и единственный прод-наблюдаемый — маркер сохранить); **(b)** детектор читает **итоговый текст turn'а**, куда попадает и **чужой текст, переданный инструментами**: процитированный пост или заголовок темы с «Подтвердите … [да/нет]» детектится, поэтому turn, который одновременно делает preview-less confirm-gated write-вызов и цитирует контент базы знаний, может вооружить preview по тексту, который никто не писал как просьбу. **Замена:** перестать предсказывать по прозе и реагировать на **детерминированный ответ пользователя** — сохранять маленький snapshot, когда turn вызвал confirm-gated write-tool без preview, и на **следующем** сообщении, если оно классифицируется как утвердительный токен, переиздать этот tool в preview-форме и вооружить `ConfirmFlow` тогда. Триггером становится **закрытый словарь** из ~десятка токенов, который кодовая база уже классифицирует надёжно (`classify_confirmation_token` / `AFFIRMATIVE_TOKENS` в [`../../tg_parser/bot/handlers.py`](../../tg_parser/bot/handlers.py), ужесточён после BUG-032), а не безграничная проза, — это убирает морфологический класс целиком и заодно закрывает канал чужого текста. Прецедент формы уже есть: `_handle_delete_intent_router` / `_handle_subscribe_intent_router` сохраняют intent-snapshot и детерминированно возобновляют его на следующем сообщении. **Цена:** в деградированном пути (LLM выбрал не ту форму вызова) пользователь подтверждает **дважды** — «да» показывает настоящий preview, второе «да» коммитит; happy path (`confirm=false`) остаётся с одним подтверждением. **Ожидаемая выгода:** три регулярки и четыре прохода по точности, скорее всего, удаляются целиком. Источник данных для приоритизации — shadow-поле `read_only_intent` из предыдущего слайса: оно же показывает, срабатывал ли детектор в проде вообще. Подробности — `BUG_LOG.md` § BUG-086, строка «Follow-up (deferred, owner decision 2026-07-25)»; трекается как [issue #359](https://github.com/AlexEfimov/TG_parser/issues/359) (заведён 2026-07-26). **Прод-данные первого окна (2026-07-26):** детектор не сработал в проде **ни разу** — значит все пять проходов по точности защищают путь, которым прод пока не ходил, и это аргумент в пользу замены, а не шестого regex-прохода. Но сам источник данных оказался слепым: `read_only_intent` пишется только на записях сработавшего guard'а, поэтому FP-rate классификатора неизмерим by construction (см. `BUG_LOG.md` § BUG-086 строку «Shadow-mode observability GAP»).

**Урок в копилку к §11.** Первые два дефекта сессии — исходный и (c′) — родились из одного и того же: **машиночитаемая подсказка, адресованная LLM, попадает в пользовательский текст и меняет поведение framework'а**. Добавляя в payload поле, которое LLM будет пересказывать, надо проверять его против всех детекторов, читающих итоговый текст turn'а. Третий дефект добавляет второй урок: **текстовый детектор нужно проверять на морфологию** — лицо и наклонение глагола различают просьбу и констатацию, а `\w*`-хвост стирает это различие. Четвёртый проход делает этот урок **доминирующим и измеримым**: из четырёх FP-векторов детектора **три оказались морфологическими** (лицо — «подтверждаю»; финитность — деепричастие «подтверждая»; наклонение/финитность — инфинитив «подтвердить») и лишь первый был лексическим (голое `confirm`). Отсюда правило для будущих проходов: русская глагольная альтернатива перечисляется **по формам**, а каждый хвост (`\w*`, `[еь]`, …) отдельно проверяется на то, какое наклонение, лицо или финитность он молча впускает.

Gate: ruff clean, **4052 passed / 22 skipped / 2 deselected** (`TEST_POSTGRES=1`; 4047 → +3 теста четвёртого прохода → 4050 → +2 теста пятого прохода).
