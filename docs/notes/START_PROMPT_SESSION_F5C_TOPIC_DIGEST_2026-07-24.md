...
| Step | Действие | Тип |
|---|---|---|
| 1 | **Migration + schema.** Новая Alembic-ревизия (chain от текущего head): `ALTER TABLE digest_subscriptions ADD COLUMN mode VARCHAR NOT NULL DEFAULT 'channel'` + `ADD COLUMN topic_ids TEXT[]` (nullable). Precedent [`20260418_add_digest_subscriptions.py`](../../migrations/versions/ingestion/20260418_add_digest_subscriptions.py). Расширить `DigestSubscription` ([`domain/models.py`](../../tg_parser/domain/models.py) L763) двумя полями + `_SELECT_COLUMNS`/`create`/`update` ([`digest_subscription_repo.py`](../../tg_parser/storage/sqlalchemy/digest_subscription_repo.py) L14/L27/L106) + port ABC ([`storage/ports.py`](../../tg_parser/storage/ports.py) L1525) sync | code+test (migration up/down) |
| 2 | **Content-selection read-path (Q6=a).** Новый read-only `TopicCardRepo.list_topics_changed_since(scope, cursor) -> list[TopicCard]` — зеркало [`list_resummarize_candidates`](../../tg_parser/storage/sqlalchemy/topic_card_repo.py) L247; key = `last_summarized_at > :cursor` (строгий `>`); scope = channel/workspace LIKE-паттерн (reuse `list_by_channels` L201) **или** explicit `topic_ids`. Write-path (`commit_resummary` L286) не трогаем | code+test |
| 3 | **Payload composition (Q2=a + Q7=a′).** На изменившуюся тему: `prior` = версия на момент cursor (Q7-fallback при purged) → `diff_topic_summaries(snapshot_from_version(prior), snapshot_from_card(card))` ([`topic_history_diff.py`](../../tg_parser/domain/topic_history_diff.py) L114/L56/L74). Fallback: nearest-available ≥ cursor, иначе genesis→current, пометка «baseline shifted (retention)»; **никогда 500** | code+test |
...
| Migration precedent (F6 schema) | [`migrations/versions/ingestion/20260418_add_digest_subscriptions.py`](../../migrations/versions/ingestion/20260418_add_digest_subscriptions.py) | revision `f6a1b2c3d4e5` | форма новой ревизии (ADD COLUMN) |
...
| Versions read-path (archival + gaps) | [`storage/sqlalchemy/topic_card_version_repo.py`](../../tg_parser/storage/sqlalchemy/topic_card_version_repo.py) | `get_two_versions` **L102**; `purge_stale` **L142** (TTL — НЕ трогать) | prior-сторона + Q7 gap-fallback |
...
- Новая Alembic-ревизия в `migrations/versions/ingestion/` (chain от текущего head — определить `alembic heads` перед написанием `down_revision`): additive `ALTER TABLE digest_subscriptions ADD COLUMN mode VARCHAR NOT NULL DEFAULT 'channel'` + `ADD COLUMN topic_ids TEXT[]` (nullable). `downgrade` дропает обе колонки. Форма — precedent [`20260418_add_digest_subscriptions.py`](../../migrations/versions/ingestion/20260418_add_digest_subscriptions.py) (idempotent guards).
...
- Новый read-only `TopicCardRepo.list_topics_changed_since(scope, cursor) -> list[TopicCard]` — зеркало [`list_resummarize_candidates`](../../tg_parser/storage/sqlalchemy/topic_card_repo.py) L247 (тот же `_row_to_model` + partial-index-friendly WHERE).
- **cursor key:** `last_summarized_at > :cursor` (строгий `>`; см. §L10-20 — repo может отдавать `>=`, фильтровать строго). `cursor is None` (first run) → reuse F6 `first_run_lookback_hours` семантику.
...
- Unit-тест: changed vs unchanged (строгий `>`); channel-scope; workspace-scope; explicit topic-scope; empty cursor lookback.

### 3.3 Payload composition (Q2=a + Q7=a′ — code+test)
- На каждую изменившуюся тему: `right` = живая карточка (`snapshot_from_card(card)` L74); `prior` = версия на момент cursor. Один вызов `diff_topic_summaries(prior_snapshot, right_snapshot)` ([`topic_history_diff.py`](../../tg_parser/domain/topic_history_diff.py) L114). **Ноль нового/ре-имплементированного diff-кода.**
- **Q7=a′ fallback** при purged промежуточной версии (через `get_two_versions` L102, absent-ключ): выбрать **nearest-available ≥ cursor**; если нет — **genesis (v1) → current** с пометкой в payload `baseline_shifted: "retention"`. ADR-0018 double-floor гарантирует genesis v1 + last-N present ⇒ fallback всегда осуществим; **никогда** не 500.
- Unit-тест: happy diff; purged-intermediate → nearest-≥-cursor; all-intermediate-purged → genesis→current + `baseline_shifted`; no-change тема исключена.
...
- MCP: extend `subscribe_digest` ([`mcp_server.py`](../../tg_parser/mcp_server.py) L3101) — доп. args `mode: str = 'channel'`, `topic_ids: list[str] | None = None`; провалидировать (topic-mode ⇒ topic_ids непустой; каждый topic visible через `assert_topic_access`). `list_digests` L3335 / `unsubscribe_digest` L3388 — reuse (вернут новые поля в модели).
- CLI: зеркало в [`cli/digest_cmd.py`](../../tg_parser/cli/digest_cmd.py) (`add` L24) — `--mode`, `--topics` (csv, reuse `_split_csv`).
- Tests: subscribe topic-mode happy; channel-mode unchanged; topic-mode без topic_ids → error; no-access topic → error-shape.
...
- **diff-API changes** — `diff_topic_summaries` / `get_two_versions` reuse as-is; нового diff surface нет.
...
- [ ] **Content-selection (Q6):** `list_topics_changed_since` отдаёт только темы с `last_summarized_at > last_digest_cursor` (строгий `>`, нет ре-включения); cursor advance **только** на успешной доставке/empty-skip; **новое** cursor-поле НЕ добавлено (reuse `last_digest_cursor`).
- [ ] **TTL-gap robust (Q7=a′):** purged промежуточная сторона → **nearest-available ≥ cursor, иначе genesis→current** с `baseline_shifted: retention`, **никогда** 500 (ADR-0018 double-floor).
...
- [ ] Visibility зеркалит `get_topic_details`/`assert_topic_access` (тема видна если user видит ≥1 из `card.sources`); topic-not-found / no-access → структурированный `{"error": ...}`, не exception.
...
6. **Q6. Cursor = (a) `topic_cards.last_summarized_at > cursor` (строгий `>`), reuse `last_digest_cursor`** — advance-only-on-success; новое cursor-поле НЕ добавляем.
7. **Q7. TTL-gap fallback = (a′) nearest-available ≥ cursor, иначе genesis→current** с пометкой «baseline shifted (retention)»; **никогда** 500. `get_two_versions` absent-ключи ⇒ fallback чист by construction (ADR-0018 double-floor).
...
1. **Q1-Q7 baked consistently** — §0/§3/§5/§7 согласованы с plan §9 и skeleton §8: Q1=both, Q2=a, Q3=a, Q4=a, Q5=a, Q6=a, Q7=a′. Ни одного места, считающего форк открытым.
...
5. **Q7=a′ fallback конкретизирован и testable** — nearest-available ≥ cursor → genesis→current → `baseline_shifted: retention`; никогда 500; §3.3 unit-тесты покрывают три ветки; опирается на ADR-0018 double-floor и absent-ключи `get_two_versions`.
6. **diff-API reused as-is** — payload = один вызов `diff_topic_summaries` (§3.3); нет нового diff surface; в OUT явно «diff-API changes».
