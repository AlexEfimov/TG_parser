...
| Versions read-path | [`storage/sqlalchemy/topic_card_version_repo.py`](../../tg_parser/storage/sqlalchemy/topic_card_version_repo.py) | **`list_by_topic` L72** (newest-first WITH `created_at`), `get_two_versions` L102, `count` L206, `purge_stale` **L142** (TTL — не трогать) | **`list_by_topic` = prior-selection** (Q7=B: новейшая версия с `created_at ≤ cursor`); `get_two_versions` (by-`version_no`) для prior НЕ используется; gaps из purge_stale |
...
| `last_digest_cursor` = `processed_at` окно | **cursor = `topic_cards.last_summarized_at > last_digest_cursor`** (reuse существующего поля, строгий `>`; reset-to-NULL при смене mode — M3) — **DECIDED Q6=a** |
...
| **#3 Cheap retrieval** | Content-selection — SQL по `last_summarized_at` **без** LLM на тему; один LLM-вызов на digest (как F6 сегодня). ⚠️ NB: в отличие от `list_resummarize_candidates` (быстр за счёт **partial-index `idx_topic_cards_resummarize_candidates WHERE new_items_since_last_summary > 0`**), фильтр `last_summarized_at > cursor` **НЕ проиндексирован** нигде, а channel-scope — non-sargable `sources_json LIKE '%"cid"%'` (идиома `list_by_channels`) ⇒ full/seq-scan. Приемлемо на текущем объёме тем; index **НЕ** заявляем. Partial-index на `last_summarized_at` — возможный deferred/OUT при росте. |
...
| **#7 Graceful degradation** | Reuse F6: bot-blocked → soft-deactivate; TTL-purged версия (Q7=B) → digest **никогда не 500 by construction** — `prior` = новейшая выжившая версия с `created_at ≤ cursor` из `list_by_topic`, иначе oldest-surviving/genesis→current, «baseline shifted (retention)» (ADR-0018 gaps). Диффим то, что физически выжило; timestamp→version SQL-lookup не нужен. |
...
| Versions read-path (`list_by_topic`) | **Read reuse** | prior-selection (Q7=B: новейшая версия с `created_at ≤ cursor` из `list_by_topic` L72); `purge_stale` (TTL) **не** трогаем. `get_two_versions` для prior **не** используется (by-`version_no` point-lookup не маппит cursor). |
...
- **Q6. Cursor semantics = (a) `topic_cards.last_summarized_at > cursor` (strict `>`), reusing `last_digest_cursor`.** Advance-only-on-success (mirrors channel digest). No new cursor column. **Mode-change reset (M3):** since `subscribe` is idempotent-upsert on `(owner_id, name)` and `last_digest_cursor` is a single TIMESTAMPTZ reused for two semantics (channel=`processed_at`, topic=`last_summarized_at`), changing an existing subscription's `mode` **resets `last_digest_cursor` to NULL** (re-triggers the first-run lookback for the new semantic) — otherwise a stale cursor of the wrong semantic → wrong first-tick window. **NULL-safety (m1):** never-re-summarized / pre-F5-C topics have `last_summarized_at = NULL`, so `NULL > cursor` is false and they are excluded by the candidate filter — **this** (not the ADR-0018 double-floor) is the real "topic never has no versions" safety mechanism. NB: the `TopicCardVersion` docstring ([`domain/models.py`](../../tg_parser/domain/models.py) ~L436, "first version is never materialised") is misleading — do not trust it as the invariant source.
- **Q7. Prior-side + TTL-gap robustness = (B) cumulative prior via `list_by_topic`.** `prior` = the summary state the user last saw at `last_digest_cursor`: from the existing `list_by_topic(topic_id, limit=keep_last_n)` ([`topic_card_version_repo.py`](../../tg_parser/storage/sqlalchemy/topic_card_version_repo.py) L72, newest-first WITH `created_at`), pick in Python the newest version with **`created_at ≤ cursor`** (at-or-before); if none survived → oldest-surviving version, else genesis (v1). `current` always = live card. This is **TRUE cumulative** change-since-last-digest (state-at-cursor → current), covering multi-bump-between-ticks (k re-summaries since last digest → all k deltas in one diff), **not** last-delta v(N−1)→current. **never 500 by construction** — we diff whatever physically survived (missing rows only shift the fallback). **Zero new repo method** (`list_by_topic` already exists). NB: `get_two_versions` (by-`version_no` point-lookup) is **not** used for prior-selection — it cannot map a cursor timestamp to a version (that was the BLOCKER); a `version_at_or_before`-style timestamp→version SQL method is **not** needed. Cost: loads ≤ keep_last_n (≤50) version rows per subscribed topic per tick — negligible at digest cadence.
...
- **diff-API changes** — `diff_topic_summaries` / `list_by_topic` reuse as-is; no new diff surface, no new repo method.
...
- [ ] **Content-selection (Q6):** returns only topics with `last_summarized_at > last_digest_cursor` (strict `>`, no re-include); cursor advances **only on successful delivery/empty-skip**. **Mode-change (M3):** changing an existing subscription's `mode` resets `last_digest_cursor` to NULL (re-triggers first-run lookback for the new semantic).
- [ ] **Cumulative prior + TTL-gap robust (Q7=B):** `prior` = newest version with `created_at ≤ last_digest_cursor` from `list_by_topic` (cumulative since-last-digest, covers multi-bump); all-purged → oldest-surviving/genesis→current with "baseline shifted (retention)" note, **never 500 by construction**. No `get_two_versions` / timestamp→version SQL method.
...
- [ ] **Visibility at BOTH subscribe and delivery (M4):** subscribe — `assert_topic_access` (topic visible if user sees ≥1 of `card.sources`); delivery — topic-mode content-selection drops any topic whose `card.sources` no longer intersects the owner's allowed channels at tick time (revoked access must not leak evolving topic summaries). Deliberate parity improvement over channel-mode F6 (which does not re-check at delivery).
...
- **Scope:** F6 subscription addendum — `mode` discriminator + `topic_ids[]`; topic-mode reuse `last_digest_cursor` (Q6=a, reset-to-NULL on mode-change); relation to ADR-0008 (delivery targets unchanged), ADR-0006 (#1/#5), ADR-0018 (Q7=B cumulative prior via `list_by_topic`).
...
1. **§8 OPEN → DECIDED** — Q1=both modes, Q2=a diff, Q3=a cron reuse, Q4=a new `topic_digest.yaml`, Q5=a schema-add, Q6=a `last_summarized_at`/reuse cursor (+reset-to-NULL on mode-change, M3), Q7=B cumulative prior via `list_by_topic` (state-at-cursor → current). Banner, §4 reuse-map, §5 scope, §6 checklist, §7 blast-radius, §9 OUT, §10 acceptance, §11 ADR — все приведены в соответствие; ни одной секции с «OPEN» форком.
...
8. **Adversarial-review resolutions applied (2026-07-24, owner-decided).** B1+M1: Q7 flipped a′→**B (cumulative prior via `list_by_topic`)** — prior = newest version with `created_at ≤ cursor`; `get_two_versions`/timestamp→version SQL-method dropped (BLOCKER: point-lookup can't map cursor→version); never-500 by construction честно (диффим то, что выжило); cumulative, не v(N−1). M2: убран ложный "indexable/cheap" claim — `last_summarized_at` unindexed + non-sargable `sources_json LIKE` ⇒ seq-scan (§6 #3, §5). M3: mode-change сбрасывает cursor в NULL (§8 Q6, §4, §10). M4: delivery-time visibility filter добавлен (§5, §10). m1: NULL-`last_summarized_at` exclusion = реальный safety mechanism + docstring-warning (§8 Q6). m2: multi-head alembic — chain от ingestion-head, не `f6a1b2c3d4e5` (§5). m3: result-DTO расширить `mode`/`topic_ids` (§5).
