...
- ✅ **DONE + ENABLED IN PROD (code 2026-07-22, включено 2026-08-11)** TTL/retention для `topic_card_versions` `(see #15 — TTL/retention)` — hybrid keep-last-N ∧ older-than-M ∧ genesis-pin, hard-DELETE zero-schema, daily cron + CLI `topic purge-versions --dry-run`, gauge/counter. Default `RESUMMARIZE_VERSION_RETENTION_DAYS=0` (kill-switch, MVP «храним всё»); **прод c 2026-08-11T23:12:51Z: `180` / `KEEP_LAST_N=50`** по owner GO с бэкапом и dry-run. Включено намеренно при `WOULD purge: 0` — механизм обкатывается вхолостую до ~конца октября 2026, вместо первой же проверки на реальном удалении. ADR: [`0018-topic-card-versions-retention.md`](../adr/0018-topic-card-versions-retention.md); plan: [`PLAN_SESSION_F5C_TTL_RETENTION_2026-07-22.md`](PLAN_SESSION_F5C_TTL_RETENTION_2026-07-22.md); skeleton (landed): [`SKELETON_F5C_TTL_RETENTION_TOPIC_CARD_VERSIONS_2026-07-20.md`](SKELETON_F5C_TTL_RETENTION_TOPIC_CARD_VERSIONS_2026-07-20.md).
- ✅ **CODE DONE (2026-07-23)** `get_topic_history_diff(topic_id, version_a, version_b)` MCP/CLI `(see #15 — diff API)` — read-only diff of a topic's evolving summary: `summary` text-diff (stdlib `difflib`) + `scope_in`/`scope_out` set-diff (added/removed/unchanged_count) over a shared pure helper `diff_topic_summaries`. Dual-source (D4): archival pairs by `version_no` (`topic_card_versions`) **and** `current`/`latest` → live card (`topic_cards`, `summary_version=N`). Default = genesis (v1) → current. Robust to post-TTL `version_no` gaps (ADR-0018): missing/purged version → typed not-found «reclaimed by retention policy», never 500. **Surface:** MCP `get_topic_history_diff`, CLI `tg-parser topic diff <id> [--version-a N] [--version-b N|current]`. Repo read-path: `TopicCardVersionRepo.get_two_versions`. No schema-change, no new deps (`difflib` stdlib). plan: [`PLAN_SESSION_F5C_DIFF_API_2026-07-23.md`](PLAN_SESSION_F5C_DIFF_API_2026-07-23.md); skeleton (landed): [`SKELETON_F5C_DIFF_API_TOPIC_HISTORY_2026-07-23.md`](SKELETON_F5C_DIFF_API_TOPIC_HISTORY_2026-07-23.md); START_PROMPT: [`START_PROMPT_SESSION_F5C_DIFF_API_2026-07-23.md`](START_PROMPT_SESSION_F5C_DIFF_API_2026-07-23.md).
- ✅ **CODE DONE (2026-07-24, with-migration)** F6 digest на topic-level summary `(see #15 — F6 digest на topic.summary)` — topic-scoped digest: `DigestSubscription` расширен `mode ('channel'|'topic')` + `topic_ids[]` (Alembic `d1e2f3a4b5c6`, additive/defaulted, no-regression). `mode='topic'` эмитит per-topic дельту эволюционирующей сводки (reuse `diff_topic_summaries`, Q2=a), не raw doc summaries. Content-selection = read-only `TopicCardRepo.list_topics_changed_since` (strict `>` на `last_summarized_at`, NULL-guard). Cursor переиспользует `last_digest_cursor` со сбросом на mode-change (M3). Prior = cumulative state-at-cursor через `list_by_topic` (Q7=B), robust к TTL-gaps (never-500). Visibility на subscribe (`assert_topic_access`) + delivery (channel-scope intersect). Surface: MCP `subscribe_digest(mode, topic_ids)` + CLI `digest add --mode/--topics`; scheduler reuse (`run_scheduled_digests_task`, mode-branch). Prompt: `prompts/topic_digest.yaml`. ADR: [`0019-f6-topic-digest-subscription-addendum.md`](../adr/0019-f6-topic-digest-subscription-addendum.md); plan: [`PLAN_SESSION_F5C_TOPIC_DIGEST_2026-07-24.md`](PLAN_SESSION_F5C_TOPIC_DIGEST_2026-07-24.md); skeleton: [`SKELETON_F5C_TOPIC_DIGEST_2026-07-24.md`](SKELETON_F5C_TOPIC_DIGEST_2026-07-24.md); START_PROMPT: [`START_PROMPT_SESSION_F5C_TOPIC_DIGEST_2026-07-24.md`](START_PROMPT_SESSION_F5C_TOPIC_DIGEST_2026-07-24.md).
- ✅ **DONE + DEPLOYED (2026-07-24, surface-only / no-migration)** Bot tools для F5-C `(see #15 — Bot tools)` — UX-сигнал получен owner'ом. Два read-only bot-tool'а в `@Tgingest_bot`, зеркалящие MCP/CLI 1:1: `get_topic_versions` (audit-trail прошлых сводок) + `get_topic_history_diff` (дельта версий, default genesis→current). Backend переиспользуется as-is (`TopicCardVersionRepo.list_by_topic`/`get_two_versions`, `diff_topic_summaries`), visibility через `allowed_channel_ids`-intersect (эквивалент `assert_topic_access`), TTL-gap → typed not-found (never-500). Оба — plain read-tools вне classifier-множеств (`_WRITE_TOOLS_REQUIRING_CONFIRM` / `_READ_TOOLS_TRACKED_FOR_CONTEXT` / `_PAGINATED_READ_TOOLS`). `bot.yaml` → `1.9.1` (capability #14). No schema-change, no migration, no new deps, no ADR. **Prod:** PR #355 (`fce2770 → b18c46e`, fast-forward), re-create `tg_bot` (BUG-078, НЕ restart); owner-verified manual smoke на темах с историей (`topic:tg:mediamedics:post:13525` и др.). START_PROMPT: [`START_PROMPT_SESSION_F5C_BOT_TOOLS_2026-07-24.md`](START_PROMPT_SESSION_F5C_BOT_TOOLS_2026-07-24.md); deploy record: [`F5C_DEPLOY_AND_WATCH.md`](../runbooks/F5C_DEPLOY_AND_WATCH.md).
...
- ✅ **DONE (2026-07-24)** **Per-topic digest:** "только новое по теме X" — `mode='topic'` + `topic_ids[]`, ADR-0019 (см. § F5-C #15 item #3 выше).
...
> **Доставка алертов: не «мгновенно», а «в течение одного интервала flush'а»**
> (по умолчанию 5 минут). Матчер живёт в процессе `tg_parser`, где `Bot` нет и
> быть не может, поэтому матч записывается там, а отправляет его отдельный
> flush в процессе бота — той же машинерией, что и батч-путь. До 2026-08-13
> второй половины не существовало вовсе: матчи копились с `notified=false`, и
> доставки не было с 2026-06-15 (BUG-095). Слово «instant» ниже читать как
> название `notify_mode`, а не как обещание синхронности.
