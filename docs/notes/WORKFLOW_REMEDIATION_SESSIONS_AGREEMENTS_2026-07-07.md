...
| S7 | O-9b + Low-диспозиции | **merged** — PR #307 → `main` `208db74` (bugbot clean, CI green); O-9b — переиспользуемый per-loop embedding-клиент в `retrieval_service.search()` + `close_embedding_client` в shutdown-сумах (FastAPI/MCP/bot), F-11 → closed; F-06/F-14/F-17/F-18 accepted, F-15/F-16 deferred ([`BUG_LOG.md`](BUG_LOG.md)); A7 → [`FUTURE_FEATURES.md`](FUTURE_FEATURES.md). Финал серии: все 18 находок ревью — fixed (S1–S6) или dispositioned (S7). |
