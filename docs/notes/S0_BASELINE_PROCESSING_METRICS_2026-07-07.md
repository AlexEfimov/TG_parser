...
| Метрика | Где определена | Labels | Текущее значение | Как снимать |
|---|---|---|---|---|
| `tg_parser_http_request_duration_seconds` | инструментатор `create_instrumentator` в `tg_parser/api/metrics.py:651–718` (namespace `tg_parser`, без subsystem — базовые имена библиотеки уже начинаются с `http_`; см. BUG-089) | `handler` (пути: `/api/v1/ask`, `/api/v1/search` — роуты в `tg_parser/api/routes/rag.py:84,116`, prefix `:16`) | **Нет сэмплов** — с рестарта API не было ни одного HTTP RAG-запроса (`..._highr_seconds_count=0.0`); RAG-трафик идёт через MCP/бота | PromQL: `histogram_quantile(0.95, sum by (le) (rate(tg_parser_http_request_duration_seconds_bucket{handler="/api/v1/ask"}[1d])))` |
