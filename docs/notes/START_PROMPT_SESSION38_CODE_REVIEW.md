# Session 38: Code Review перед RAG

**Дата:** [дата запуска]  
**Тип сессии:** Code review и подготовка к P5 (RAG)  
...
Провести всестороннее ревью кодовой базы TG_parser перед переходом к P5 (RAG-интеграция):
- Оценить качество кода, архитектуру, именование, DRY
- Найти потенциальные баги, техдолг, неоптимальности
- Предложить рефакторинги до начала работы с embeddings/vector storage
- Убедиться, что фундамент крепкий для следующего этапа
...
- [ ] Оценить: нужен ли единый Database lifecycle (singleton vs per-call creation)?
...
- [ ] Naming: `SQLite*` repos при работе с PostgreSQL — переименовать?
...
- [ ] Спроектировать retrieval pipeline
