...
| `ResummarizeRefusalCooldownPoisonPill` | `alertstate="pending"` — ожидаемо: значение 14 ≥ 12, но `for: 6h`. Это остаток окна ДО лечения темы, а не новый poison-pill. **Подтверждено замером:** в `10:46:04Z` счётчик перешёл `12.003 → 11.003`, алерт исчез из `ALERTS` и до `firing` **не дошёл** (проверено ещё трижды до 11:16Z — пусто). Спад идёт **ступенями по мере выпадения событий из 24h-окна**, а не плавно: мои прогнозы точного времени («~09:50Z») дважды промахнулись именно поэтому — значение стояло на 12.003 более получаса, затем упало разом |
