...
- ✅ **Исполнено 2026-08-05.** Исключить `outcome="refusal_cooldown"` из recording rule / gate (чтобы ratio отражал реальные re-summarize). Исключён из **обеих** частей дроби; `ratio14d` ≈0.989 → ≈0.90. Заодно принято owner-решение по порогу: gate-алерт `ResummarizeAgeTriggerGateF5CPhase2` **снят** (вариант V1 — вердикт закрыт, вечно-красный info-алерт вреден), взамен добавлен компенсирующий сигнал `tg:resummarize_refusal_cooldown:count24h` + alert `ResummarizeRefusalCooldownPoisonPill`.
