...
- Исключить `outcome="refusal_cooldown"` из recording rule / gate (чтобы ratio отражал реальные re-summarize).
- Или не выбирать cooldown-темы в `list_resummarize_candidates` (не жечь слот тика на known poison-pill).
- Опц. `RESUMMARIZE_REFUSAL_FALLBACK_STAGE` для попытки вылечить тему другим провайдером.
