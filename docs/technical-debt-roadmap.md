...
**Пункт 7 закрыт.** Остаточное: скрипты хоста не версионируются (лежат в снапшоте `files` последним состоянием, без истории) — при желании закрывается приватным репозиторием оператора.
