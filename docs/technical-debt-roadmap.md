...
**Осталось открытым:** требование Т-6 (устойчивость к компрометации хоста) **не выполнено** — через S3 ключ хоста обязан иметь `deleteFiles`, а удаление с `versionId` необратимо, поэтому версионирование бакета от целенаправленного взломщика не защищает. Проверено экспериментом на канареечном объекте. Нужен выбор владельца из трёх вариантов (ADR § Update). До этого ADR остаётся `Proposed`.
